...
// PnP.Core.Model.SharePoint also declares a BasePermissions - an internal, context bound data model
// type - so the provisioning POCO has to be disambiguated explicitly.
...
namespace PnP.Core.Provisioning.Test.Offline.Model
{
    ...
    /// </summary>
    /// <remarks>
    /// The expected values below were generated from the real CSOM implementation
    /// (Microsoft.SharePointOnline.CSOM 16.1.26914.12004) rather than written from the documentation,
    /// because two behaviours are genuinely surprising and both are relied upon by the schema resolvers:
    ///
    /// - <c>Set(FullMask)</c> does not set every bit. It sets high = 0x7FFF, low = 0xFFFF, so permissions
    ///   above bit 15 of either word - ManageWeb, EnumeratePermissions and others - report false afterwards.
    /// - <c>Clear(FullMask)</c> and <c>Clear(EmptyMask)</c> are no-ops. Only ClearAll() empties the mask.
    ///
    /// These tests are offline: they need no SharePoint connection, and no reference to CSOM.
    /// </remarks>
    [TestClass]
    public class BasePermissionsTests
    {
        ...
        {
            // CSOM treats EmptyMask as always present, including on a fresh instance.
        ...
        {
            // Deliberately asserting the CSOM quirk: FullMask sets high = 0x7FFF, low = 0xFFFF, so
            // anything above bit 15 in either word is NOT granted. Changing this would silently change
            // which permissions a template carrying Rights="FullMask" round trips to.
            BasePermissions permissions = new BasePermissions();
        ...
        {
            // Set(EmptyMask) is a reset, not a no-op - and so is NOT the inverse of Clear(EmptyMask).
            // Testing this on a fresh instance would pass either way, which is precisely how the
            // original implementation got this wrong.
            BasePermissions permissions = new BasePermissions();
            permissions.Set(PermissionKind.ViewListItems);
            permissions.Set(PermissionKind.ManageWeb);
            permissions.Set(PermissionKind.UseClientIntegration);
        ...
        {
            // EnumeratePermissions is high bit 30, which FullMask does not include - so setting
            // FullMask afterwards must drop it rather than merge with it.
            BasePermissions permissions = new BasePermissions();
        ...
        {
            // CSOM does nothing for either. Clear(FullMask) in particular reads like "remove everything"
            // but is not - ClearAll() is the way to empty a permission set.
            BasePermissions permissions = new BasePermissions();
            permissions.Set(PermissionKind.ViewListItems);
            permissions.Set(PermissionKind.ManageWeb);
            permissions.Set(PermissionKind.UseClientIntegration);
