...
namespace PnP.Core.Provisioning.Test.Offline.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// Runs offline against a stub <see cref="HttpMessageHandler"/> that records the request rather
    /// than sending it. The contract here is what a customer's endpoint receives, so it is worth
    /// pinning even though nothing in SharePoint is involved.
    /// </remarks>
    [TestClass]
    public class WebhookSenderTests
    {
        ...
        private static ProvisioningTemplateWebhook Webhook(ProvisioningTemplateWebhookMethod method, string url = "https://contoso.example/hook")
        {
            return new ProvisioningTemplateWebhook
            {
                ...
                // Synchronous, so the test can assert after the await rather than racing delivery.
                Async = false,
            };
        }
        ...
        public async Task Get_UrlEncodesParameterNamesAndValues()
        {
            ...
            using (var client = new HttpClient(handler))
            {
                ...
                // The delimiters are what matter: an unencoded '&' would split the value into a
                // second parameter, and an unencoded '=' would corrupt the pair. Uri canonicalizes
                // %20 back to a literal space in the query string, so asserting on the escaped
                // space would be testing System.Uri rather than the sender.
        ...
        public async Task Get_TruncatesAnExceptionToItsMessage()
        {
            ...
            using (var client = new HttpClient(handler))
            {
                ...
                // A GET carries only the message - a full stack trace would blow the url length limit.
        ...
        public async Task Post_SendsAnXmlBodyWhenAsked()
        {
            ...
            using (var client = new HttpClient(handler))
            {
                ...
                // This format never worked in PnP Framework: it handed the parameter dictionary to
                // XmlSerializer, which refuses any IDictionary, so every XML webhook threw
                // NotSupportedException - and the surrounding catch only handled
                // HttpRequestException, so it failed the whole provisioning run. Guards the fix.
        ...
        public async Task Post_CarriesTheFullExceptionDetail()
        {
            ...
            using (var client = new HttpClient(handler))
            {
                ...
                // Unlike a GET there is no url length limit, so the POST body carries ToString().
        ...
        public async Task Parameters_AreResolvedAgainstEachOther()
        {
            ...
            using (var client = new HttpClient(handler))
            {
                // The url may reference the webhook's own parameters by name; the sender resolves
                // those with an internal SimpleTokenParser after the template level parse.
                ProvisioningTemplateWebhook webhook = Webhook(ProvisioningTemplateWebhookMethod.GET,
