...
// Fourth name collision of the migration: RegionalSettings, after SearchBoxInNavBar, Feature and
// Theme. The template model and PnP Core name the same concepts throughout - alias by reflex.
...
{
    /// <summary>
    /// Live coverage for <c>ObjectRegionalSettings</c> (<b>T1</b>) and
    /// <c>ObjectSupportedUILanguages</c> (<b>T2</b>).
    /// </summary>
    [TestClass]
    public class RegionalAndLanguageLiveTests : LiveTestBase
    {
        ...
        public async Task RegionalSettings_ExtractReadsThePropertiesPnPCoreCannotModel()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                // The point of the raw-REST read: these four are NOT on PnP Core's
                // IRegionalSettings at all, so a model-based extraction would silently return
                // zeroes for them.
                Assert.AreNotEqual(0, settings.LocaleId, "LocaleId should never be 0 on a real site.");
                // Deliberately NOT asserting these are non-zero: a site may legitimately have never
                // configured working days, and an earlier version of this test failed on exactly that
                // - an assumption about the site dressed up as an assertion about the code.
                // LocaleId above is the real signal that the raw REST read worked.
            }
        }
        ...
        public async Task RegionalSettings_RoundTripsAChangedWorkingDay()
        {
            using (PnPContext context = await GetClassicContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    // Change one obviously reversible thing.
        ...
        public async Task RegionalSettings_UmAlQuraFailsLoudlyRatherThanSilently()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                // The T5b decision made concrete. SharePoint does not support this calendar and
                // PnP Core deliberately omits it, so applying a different one silently would
                // misrepresent the template. A named error is the correct outcome.
        ...
        {
            // Run against the classic site: the modern one already has 51 languages enabled, so
            // adding one proves little and removing the wrong one would be disruptive.
            using (PnPContext context = await GetClassicContextAsync().ConfigureAwait(false))
            {
                ...
                finally
                {
                    try
                    {
                        // Restore by applying the original list - the handler removes anything the
                        // template omits, which is exactly what is wanted here.
