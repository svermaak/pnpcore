...
namespace PnP.Core.Provisioning.Test.Live.Handlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <b>Every page these tests create is named with the suite prefix and deleted in a
    /// <c>finally</c>.</b> A leaked page is not merely untidy - it stays in the site's pages library
    /// and shows up in the next extraction, which then asserts against content a previous test run
    /// left behind.
    /// </remarks>
    [TestClass]
    public class ClientSidePagesLiveTests : LiveTestBase
    {
        ...
        public async Task ClientSidePages_ExtractSurveysTheLibraryAndSkipsAnUncustomisedHomePage()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                {
                    // An untouched default home page has no page definition stored on its list item.
                    // Exporting it would produce an empty page that overwrites a perfectly good
                    // default on the target, so the handler deliberately skips it.
        ...
        public async Task ClientSidePages_ExtractTokenizesListAndSiteIdsOutOfWebPartData()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    // Build the page through PnP Core rather than the engine, so the ids in the web
                    // part's data are unambiguously real ones this site owns.
                    ...
                    // The whole point of extraction is portability: a raw id left behind means the
                    // applied page points back at the site it came from.
            ...
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    using (PnPContext fresh = await GetContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // Assert on the text itself: a control that exists but is empty is the
                        // failure mode a "two controls" count alone would miss.
        ...
        public async Task ClientSidePages_RoundTripsAPageThroughExtractAndApply()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    // 1. Create a page with a known shape.
                    ...
                    // 2. Extract everything, and find it again.
                    ...
                    // 3. Apply the extracted page back under a new name - the actual round trip.
        ...
        public async Task ClientSidePages_RefusesToOverwriteWhenTheTemplateSaysNotTo()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    // Now apply again with a different title and Overwrite off.
        ...
        public async Task ClientSidePages_UniquePagePermissionsWarnRatherThanBeingDroppedSilently()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    // Security is a phase 6 handler. Until then the engine must SAY it did not apply
                    // this rather than producing a page that looks provisioned and is world readable.
