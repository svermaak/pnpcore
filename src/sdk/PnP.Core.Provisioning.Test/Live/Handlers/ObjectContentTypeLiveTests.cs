...
{
    /// <summary>
    /// Live coverage for <c>ObjectContentType</c>, including document sets (<b>T15</b>).
    /// </summary>
    /// <remarks>
    /// A leaked content type cannot simply be deleted if a list is using it, so everything here is
    /// created at site scope only, prefixed, and removed in a <c>finally</c> - content types first,
    /// then the columns they link to, because a column still linked from a content type cannot go.
    /// </remarks>
    [TestClass]
    public class ObjectContentTypeLiveTests : LiveTestBase
    {
        // Derived from Item (0x01) so the tests do not depend on any other content type existing.
        private const string ContentTypeId = "0x0100A9C1B2D34E5F6071829304A5B6C7D8E9";
        // 0x0120D520 (Document Set) + "00" + exactly 32 hex characters - 44 characters in total.
        // A shorter tail is rejected as "Value does not fall within the expected range", which names
        // nothing useful, and it is easy to miscount by two.
        ...
        public async Task ContentTypes_PutsTheColumnsInTheTemplatesOrder()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                try
                {
                    // Second before First. A derived content type gets its parent's columns first
                    // with its own appended, so without the reorder the template's order is lost -
                    // and that order is what the new and edit forms use.
        ...
        /// </summary>
        /// <remarks>
        /// <b>This test fails if PnP Core is fixed</b>, which is the point: at that moment the CSOM
        /// request can be dropped in favour of <c>IContentType.UpdateAsync()</c> for the property
        /// half - though <c>UpdateChildren</c> would still need it.
        /// </remarks>
        [TestMethod]
        [TestCategory("Live")]
        [TestCategory("Handlers")]
        public async Task ContentTypes_PnPCoreCannotUpdateAContentTypeReadFromTheCollection()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    // (a) the object AddAsync returned - this is the only case PnP Core's own tests
                    // cover, and it works.
                    try
                    {
                    ...
                    // (b) an object read back from Web.ContentTypes - which is what a provisioning
                    // run almost always holds, because the content type usually already exists.
                    try
                    {
                        using (PnPContext fresh = await GetContextAsync(1).ConfigureAwait(false))
                        {
        ...
        public async Task ContentTypes_Diagnose_IsTheDocumentSetsFeatureOn()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                // The Document Sets site collection feature.
        ...
        {
            // Separates two explanations for the re-apply failure: stale client state on the context
            // that did the first apply, versus something about the update itself. If this passes and
            // the same-context version fails, it is staleness.
            try
            {
        ...
        public async Task ContentTypes_CreatesADocumentSetWithItsSharedColumns()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    // Backlog T15. PnP Core models the whole document set - allowed content types,
                    // shared columns, welcome page columns, default contents - so this is one call
                    // rather than the four separate collections CSOM exposed.
        ...
        public async Task ContentTypes_ExtractReadsTheSitesOwnContentTypesAndSkipsTheBuiltInOnes()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    // Built-in content types must not come along - a template full of Item, Document
                    // and Folder is unusable.
        ...
        /// </summary>
        /// <remarks>
        /// <c>CsomServiceException.Message</c> is a bare banner; the server's own text is on the
        /// error object, which <see cref="LiveTestBase.Describe"/> renders.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Order matters: a site column that is still linked from a content type cannot be deleted,
        /// and the resulting error is about the column rather than the link.
        /// </remarks>
        private static async Task CleanUpAsync()
        {
            // Sweeps by NAME, not only by the id constants. An earlier run that used a different id
            // leaves a content type with the same name behind, and SharePoint then refuses the next
            // create with "a duplicate content type was found" - which reads like a bug in the
            // handler rather than debris from a previous run.
