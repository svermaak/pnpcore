...
namespace PnP.Core.Provisioning.Test.Live.Handlers
{
    ...
    /// </summary>
    /// <remarks>
    /// The behaviour worth pinning is <b>idempotence</b>. A template applied twice must not double
    /// its content when it declares a key column, and must update the matched row rather than
    /// leaving it stale. Both directions are asserted on the row <em>count</em>, because an
    /// assertion that only checks "the row is there" stays true through a duplicate.
    /// </remarks>
    [TestClass]
        ...
        public async Task DataRows_AKeyColumnMakesASecondApplyUpdateRatherThanDuplicate()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    // Same key, different payload. This must find the existing row and overwrite it.
                    ...
                    using (PnPContext fresh = await GetContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // The count is the assertion that matters: "the row exists" would have been
                        // true even if the second apply had added a duplicate.
