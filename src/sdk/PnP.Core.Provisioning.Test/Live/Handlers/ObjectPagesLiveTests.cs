...
{
    /// <summary>
    /// Live coverage for <c>ObjectPages</c> (classic wiki pages, <b>T18</b>).
    /// </summary>
    /// <remarks>
    /// <para>Classic wiki pages need a site with scripting enabled, so these run against the
    /// classic STS#0 site and report themselves inconclusive if it is NoScript rather than
    /// pretending to have tested something.</para>
    /// <para>The assertion that earns its place is on the <b>wiki field</b>. A page file can be
    /// created successfully and still be unusable - if the layout HTML is wrong or missing, the
    /// page opens with no editable zones, and every check that only looks for the file passes.</para>
    /// </remarks>
    [TestClass]
    public class ObjectPagesLiveTests : LiveTestBase
    {
            ...
            using (PnPContext context = await GetClassicContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    using (PnPContext fresh = await GetClassicContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // layoutsData is header,footer,columns. TwoColumnsHeader is true,false,2 -
                        // asserting on it is what distinguishes "a layout was written" from "the
                        // layout the template asked for was written".
            ...
            using (PnPContext context = await GetClassicContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    using (PnPContext fresh = await GetClassicContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // false,false,3 is the three-column layout. If overwrite had silently done
                        // nothing, this would still be the one-column layout and the count check
        ...
        public async Task Pages_AreSkippedOnANoScriptSite()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                // The warning should point at the modern alternative, not just refuse - a template
                // author on a modern site needs to know what to use instead.
        ...
        private static async Task SweepAsync()
        {
            ...
            {
                // Absent is the common case when the test failed before creating it.
