...
{
    /// <summary>
    /// Live coverage for <c>ObjectTermGroups</c> and backlog <b>T10</b>.
    /// </summary>
    /// <remarks>
    /// <para><b>These tests write to the tenant-wide term store.</b> Every group they create is
    /// named with the suite prefix and deleted in a <c>finally</c>; a leaked term group is visible
    /// to the whole tenant, not just to the test site.</para>
    /// <para>The assertion that matters most is <b>GUID preservation</b>. Spike S1 found that Graph
    /// reassigns term ids, which is why decision D7 sends every taxonomy write through CSOM - so a
    /// test that only checks "a term called X exists" would pass with the bug fully present.</para>
    /// </remarks>
    [TestClass]
    public class ObjectTermGroupsLiveTests : LiveTestBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>Per test, not per class.</b> The term store is tenant-wide and a term group name
        /// must be unique across it, so a shared name means a test whose teardown is incomplete -
        /// or merely slow - fails the <em>next</em> test with "Group names must be unique". That
        /// points at the wrong test entirely, and cost a diagnosis round here.</para>
        /// <para>The ids are still <b>fixed per test rather than random</b>: preserving the exact id
        /// the template supplied is what decision D7 is for, so asserting on a value the test itself
        /// generated would prove nothing about the CSOM path that a random value would not also
        /// satisfy by accident.</para>
        /// </remarks>
            ...
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    using (PnPContext fresh = await GetContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // Decision D7 exists for exactly this. Graph mints its own id; the CSOM path
                        // preserves the template's, and every taxonomy column default and list item
                        // value in a real template refers to it.
            ...
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    using (PnPContext fresh = await GetContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // Sets and terms load in separate calls: PnP Core's Graph term store refuses
                        // a nested QueryProperties across these collections.
            ...
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    using (PnPContext fresh = await GetContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // The count is the assertion. A second apply that re-created the terms under
                        // new ids would leave four, and every "the term exists" check would pass.
            ...
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    using (PnPContext fresh = await GetContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // The nesting has to survive: a flat extract of a hierarchy re-applies as a
                        // flat term set, which is a silent structural change.
        ...
        /// </summary>
        /// <remarks>
        /// <para>Deliberately not the shared <c>CleanUpLeakedTermGroupsAsync</c>: that one swallows
        /// a failed delete on purpose, leaving it to the hygiene test to name later. That is fine
        /// for a leftover from a crashed run and useless here - a group that survives teardown makes
        /// the <em>next</em> test fail with "Group names must be unique", which points at the wrong
        /// thing entirely.</para>
        /// <para>A group cannot be deleted while it holds term sets, and a set cannot be deleted
        /// while it holds terms, so the walk is depth first.</para>
        /// </remarks>
