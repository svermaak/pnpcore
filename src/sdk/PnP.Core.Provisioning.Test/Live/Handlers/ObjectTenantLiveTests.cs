...
{
    /// <summary>
    /// Live coverage for <c>ObjectTenant</c> - backlog <b>T7</b>, <b>T8</b> and the reachable half of
    /// <b>T13</b>.
    /// </summary>
    /// <remarks>
    /// <para><b>Only the parts that create and delete their own artefacts are tested here.</b> Site
    /// scripts, site designs, tenant themes and storage entities are named objects that this test
    /// makes and removes, in the same shape as the term group tests - each name carries the suite
    /// prefix so a leaked one is obvious, and teardown runs in a <c>finally</c>.</para>
    /// <para><b>Sharing settings and the CDN are deliberately not tested.</b> They are not artefacts;
    /// they are the tenant's live configuration, they affect every site and every user, and there is
    /// no "put it back" that is honestly safe to run automatically. Their code paths are covered by
    /// reading and by the offline payload tests, and they need a throwaway tenant - not a throwaway
    /// object - to exercise properly. That is recorded rather than quietly skipped.</para>
    /// </remarks>
    [TestClass]
    public class ObjectTenantLiveTests : LiveTestBase
    {
        ...
        public async Task SiteScriptsAndDesigns_AreCreatedAndPublishTheirIdsAsTokens()
        {
            ...
            // Aliased: the provisioning model has its own File and Directory types, and the
            // enclosing namespace wins over a using directive.
            ...
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                using (admin)
                {
                    // Belt and braces. Each test cleans up after itself, but a run that dies - or one
                    // whose teardown could not read the list, which is exactly what happened while
                    // the response envelope was being parsed wrongly - leaves tenant-wide objects
                    // behind that nothing else sweeps.
                    ...
                    try
                    {
                        ...
                        var design = new SiteDesignModel
                        {
                            ...
                            // By title, not by id: the id does not exist until the script above is
                            // created, and the token is how the two are joined. If script processing
                            // did not run first, or did not publish its token, this arrives at
                            // SharePoint as the literal text and the design is rejected.
                        ...
                        // The assertion that means something. A design that exists but references no
                        // script would satisfy "was it created" and be useless - and that is exactly
                        // what happens if the token did not resolve.
        ...
        public async Task StorageEntities_AreWrittenToTheAppCatalogAndReadBack()
        {
            ...
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    // Attempted first, diagnosed afterwards. An earlier version checked NoScript up
                    // front and skipped - which was wrong twice over: the app catalog is NoScript
                    // whether or not the write is permitted, so the check both hid a passing test
                    // once the tenant allowed property bag updates, and would have gone on hiding
                    // any real defect behind a plausible excuse.
                    await ApplyAsync(context, template, catalogUri).ConfigureAwait(false);

                    // Read through a fresh context bound to the catalog, not through the applying
                    // one: the point is what SharePoint stored.
        ...
        /// </param>
        /// <remarks>
        /// <b>The tenant-configuration escape only applies to access denied.</b> Anything else is a
        /// failure, so a real defect cannot hide behind "the tenant is not set up for this" - which
        /// is the trap the up-front NoScript check fell into.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Designs first: a design that references a script does not stop the script being deleted,
        /// but leaving a design pointing at nothing is worse than leaving neither.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Designs before scripts, and both in a <c>finally</c>: these are tenant-wide objects that
        /// nothing else sweeps up, so a failed assertion must not leave one behind.
        /// </remarks>
