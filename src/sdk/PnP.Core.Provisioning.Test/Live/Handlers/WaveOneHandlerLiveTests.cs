...
// The provisioning model and PnP Core both declare Feature and Theme. Aliased so neither can be
// picked up by accident - this collision is systematic and hits every handler that names a
// template artefact after a SharePoint one.
    ...
    public class WaveOneHandlerLiveTests : LiveTestBase
    {
        ...
        public async Task SearchSettings_ExtractDoesNotThrowOnASiteWithNoCustomization()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                // A site with no search customization legitimately yields nothing. What matters is
                // that extraction completed - the handler swallows the "not available here" errors
        ...
        public async Task Features_ActivateAndDeactivateAWebFeature()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                // Minimal Download Strategy: a web-scoped, side-effect-free feature that exists on
                // every site and is safe to toggle.
        ...
        public async Task Features_AnUnavailableFeatureWarnsRatherThanFailingTheRun()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                // A template is regularly applied to sites that differ from its source. One
                // unavailable feature must not abort everything else in the run.
                var template = new ProvisioningTemplate();
        ...
        public async Task Theme_AppliesABuiltInThemeByName()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                // Built-in themes are the one case that works before backlog T13 lands.
        ...
        public async Task Theme_APaletteWarnsThatItWasNotApplied()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                // Until T13, a palette cannot be applied. The handler must SAY so - silently
                // leaving the site unthemed while reporting success is the failure mode this
                // migration has hit repeatedly.
                var template = new ProvisioningTemplate
                {
