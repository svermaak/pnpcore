...
{
    /// <summary>
    /// Live coverage for <c>ObjectSiteSecurity</c>, including backlog <b>T3</b>.
    /// </summary>
    /// <remarks>
    /// <para>These tests run against the <b>no-group</b> site. A group-connected site's owner and
    /// member groups are managed by the Microsoft 365 group behind it, so reassigning them there
    /// would be fighting the platform rather than testing the handler.</para>
    /// <para>Nothing here touches site collection administrators. Clearing them is destructive, and
    /// granting them to a real account on a shared tenant is not something a test should do
    /// silently - that path is exercised by the warning branch only.</para>
    /// </remarks>
    [TestClass]
    public class ObjectSiteSecurityLiveTests : LiveTestBase
    {
            ...
            using (PnPContext context = await GetNoGroupContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    using (PnPContext fresh = await GetNoGroupContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // The count is the assertion. "The group exists" stays true through a
                        // duplicate, and SharePoint happily holds two groups with the same title.
        ...
        {
            // The classic STS#0 site, not the communication site: a modern site is NoScript, and
            // SharePoint refuses to reassign an associated group there. The handler correctly warns
            // and does nothing, so running this on a NoScript site would assert against a no-op.
            using (PnPContext context = await GetClassicContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    using (PnPContext fresh = await GetClassicContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // Backlog T3: IWeb.AssociatedOwnerGroup is get-only and has no REST
                        // equivalent, so this passing is the whole proof that the CSOM request works.
            ...
            using (PnPContext context = await GetNoGroupContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    using (PnPContext fresh = await GetNoGroupContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // The permissions are the point of a permission level; asserting only that
                        // one exists would pass for an empty level.
            ...
            using (PnPContext context = await GetNoGroupContextAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    using (PnPContext fresh = await GetNoGroupContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // Off by default, and the handler honours that: a site's groups are often
                        // tenant-specific and an extract should not assume you want them.
                        ...
                        // Only custom levels belong in a template: re-applying "Full Control" fails,
                        // because every site already has it.
        ...
        /// </summary>
        /// <remarks>
        /// Most of these tests use the communication site; the associated-group one has to use the
        /// classic site, and its cleanup has to follow it there or it leaves groups behind.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// The order matters: deleting a group that is still the associated owner group leaves the
        /// site with no owners group at all, which the next run cannot repair.
        /// </remarks>
