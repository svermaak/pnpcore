...
namespace PnP.Core.Provisioning.Test.Live.Handlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <b>Every assertion here is chosen so an empty or default-valued model fails it.</b> That is
    /// the lesson from the regional settings episode: a test asserting "extraction returned an
    /// object" passed for weeks against a read that was returning nothing but fallbacks, and the
    /// resulting all-zero model then made a perfectly good write look broken.
    /// </remarks>
    [TestClass]
    public class WebChromeAndCustomActionsLiveTests : LiveTestBase
    {
        ...
        public async Task SiteHeader_ExtractReadsTheRealChromeOptions()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                // Compare against the live chrome rather than against constants: this catches a read
                // that returns plausible-looking defaults, which is exactly what a broken one does.
        ...
        public async Task SiteHeader_AppliesLayoutAndMenuStyleAndRestoresThem()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                // Flip to something demonstrably different from what is there now.
        ...
        public async Task WebSettings_ExtractReadsValuesOnlyAWorkingReadCanProduce()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                // Every real site has a welcome page and a master page. If either is empty the read
                // is broken, not the site - which is precisely how the regional settings bug hid.
        ...
        public async Task CustomActions_AddUpdateAndRemoveAWebScopedAction()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                // An SPFx application customizer - the one shape a NoScript site accepts, so this
                // test verifies the same path on every site type.
                ...
                try
                {
                    ...
                    // Applying the same name again must update rather than duplicate.
                    ...
                    // And Remove=true must delete it.
        ...
        {
            // The classic site, because the default test site is NoScript and property bag writes
            // are refused there - which is the handler behaving correctly, and so proves nothing.
            using (PnPContext context = await GetClassicContextAsync().ConfigureAwait(false))
            {
                ...
                using (PnPContext fresh = await GetClassicContextAsync(1).ConfigureAwait(false))
                {
                    ...
                    // The bookkeeping entries must not survive into the extracted template.
