...
namespace PnP.Core.Provisioning.Test.Live.Handlers
{
    ...
    /// </summary>
    /// <remarks>
    /// The files go into a library this suite creates and deletes, rather than into Site Assets or
    /// Site Pages. Uploading test content into a library the tenant actually uses leaves debris that
    /// the next extract picks up, and a failed sweep would leave it there permanently.
    /// </remarks>
    [TestClass]
        ...
        public async Task Files_UploadsAFileIntoALibraryTheSameTemplateCreates()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    // Publishing only exists on a library with minor versions on. Without this the
                    // handler warns and moves on - which is correct behaviour, but it means the
                    // publish path itself would never be exercised.
                    ...
                    using (PnPContext fresh = await GetContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // The bytes matter as much as the name: an upload that creates an empty file
                        // is indistinguishable from a good one if only the name is asserted.
        ...
        public async Task Files_AMissingSourceFileFailsLoudly()
        {
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    // A template referencing a file the package does not carry is a broken template.
                    // Skipping it quietly would produce a site that looks provisioned and is not.
