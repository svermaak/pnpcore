...
namespace PnP.Core.Provisioning.Test.Live.Csom
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>PnP Framework skips the assignment on a NoScript site and logs <i>"Won't modify
    /// associated group configuration since the template is applied to a NoScript site"</i>. That
    /// guard is a <b>choice in PnP Framework's code</b>, not something SharePoint documents -
    /// NoScript removes <c>AddAndCustomizePages</c>, and assigning an associated group is a
    /// <c>Web.Update()</c>, which has no obvious reason to need it.</para>
    /// <para>Every modern site is NoScript. If the guard is over-broad, the port would refuse a
    /// perfectly legal operation on essentially every site a template targets - so this is worth
    /// one probe rather than an inherited assumption.</para>
    /// </remarks>
    [TestClass]
            ...
            using (PnPContext context = await GetNoGroupContextAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    using (PnPContext fresh = await GetNoGroupContextAsync(1).ConfigureAwait(false))
                    {
                        ...
                        // Accepting the request and not applying it is the outcome worth catching:
                        // it would make the guard look unnecessary while the behaviour stayed broken.
                ...
                {
                    // Only when the assignment actually landed. Restoring after a refusal is a
                    // second guaranteed access-denied, and its "COULD NOT RESTORE" line reads as
                    // though the probe left the site broken when it changed nothing at all.
