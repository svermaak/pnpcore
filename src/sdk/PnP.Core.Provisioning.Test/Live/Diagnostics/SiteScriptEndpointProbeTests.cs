...
namespace PnP.Core.Provisioning.Test.Live.Diagnostics
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Written because <c>ObjectTenant</c>'s first live run created a site script and a site
    /// design, reported no problem, and left neither behind - the signature of a response this code
    /// parses into nothing rather than of a call that failed. Guessing at the envelope shape would
    /// have cost several rounds; printing it costs one.</para>
    /// <para>A probe, not an assertion. It asserts only that the endpoint answered, and leaves the
    /// shape to be read.</para>
    /// </remarks>
    [TestClass]
    public class SiteScriptEndpointProbeTests : LiveTestBase
    {
            ...
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                using (admin)
                {
                    ...
                    try
                    {
                        ...
                        // The probe's one assertion: whatever the envelope is, the endpoint has to
                        // hand back an id, or nothing downstream can reference the script.
        ...
        /// </summary>
        /// <remarks>
        /// <c>SetStorageEntity</c> and <c>RemoveStorageEntity</c> both report success, and the read
        /// that follows says the key does not exist. Unwrapping the verbose envelope did not change
        /// that, so what the write and the read actually exchange is worth seeing rather than
        /// reasoning about.
        /// </remarks>
        [TestMethod]
        [TestCategory("Live")]
            ...
            using (PnPContext context = await GetContextAsync().ConfigureAwait(false))
            {
                ...
                using (PnPContext catalog = await context.CloneAsync(catalogUri).ConfigureAwait(false))
                {
                    ...
                    try
                    {
                        ...
                        // Read twice: once immediately, once from a second context, so a caching
                        // answer is distinguishable from a store that never took the value.
                        ...
                        using (PnPContext freshCatalog = await fresh.CloneAsync(catalogUri).ConfigureAwait(false))
                        {
                            ...
                            // The tenant properties view of the same thing - a storage entity is a
                            // tenant property, and the admin API may see what the web API does not.
