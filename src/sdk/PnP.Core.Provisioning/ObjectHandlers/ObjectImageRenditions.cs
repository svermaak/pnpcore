...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T20</b>. <c>SiteImageRenditions</c> is CSOM only - there is no REST or Graph
    /// equivalent - so this rides on the two requests built in phase 4.</para>
    /// <para><b>Renditions are merged, not replaced.</b> <c>SetRenditions</c> takes the whole list,
    /// so the existing ones are read first and the template's added to them. Sending only the
    /// template's would delete every rendition the site already had, and existing pages reference
    /// them by name.</para>
    /// <para><b>Publishing has to be on.</b> The template's <c>AutoCheckRequirements</c> decides
    /// what happens when it is not: skip quietly, activate the features, or fail. That is the
    /// template author's call, not this handler's.</para>
    /// </remarks>
