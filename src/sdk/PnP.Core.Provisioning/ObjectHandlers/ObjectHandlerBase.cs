...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Async and context-typed.</b> PnP Framework's handlers took a CSOM <c>Web</c> and
    /// ran synchronously. These take a <see cref="PnPContext"/> and return tasks. Every handler in
    /// the engine derives from this, so the shape is settled once here rather than 41 times.</para>
    /// <para><b>The parser is threaded, not copied.</b>
    /// <see cref="ProvisionObjectsAsync"/> receives a <see cref="TokenParser"/> and returns one.
    /// The engine feeds each handler's return value into the next, so a handler that registers a
    /// token - a list id, a field id - makes it available to everything downstream. Returning a
    /// different instance than the one passed in is legal but almost always a mistake.</para>
    /// </remarks>
    internal abstract class ObjectHandlerBase
    {
        ...
        protected string TokenizeListView(string xml, IList list, IWeb web)
        {
            ...
            // ListContentTypeIdToken.CreateToken regex-escapes its input, which escapes spaces
            // with a backslash and so breaks the JSON in NewDocumentTemplates. Pull that element
            // out, substitute into it separately, and put it back afterwards.
        ...
        /// </summary>
        /// <remarks>
        /// PnP Framework used Newtonsoft's JObject to mutate this in place. System.Text.Json's
        /// DOM is read-only, so the array is re-emitted rather than edited - same result, and the
        /// JSON is compact either way.
        /// </remarks>
        private static string TokenizeNewDocumentTemplateUrls(string json, IWeb web)
        {
            ...
            {
                // Not valid JSON after all - leave it exactly as found, as PnP Framework did.
                return json;
            }
        }
        ...
        protected string TokenizeXml(string xml, IWeb web)
        {
            ...
            // On the root site collection ServerRelativeUrl is just "/", so replacing every "/"
            // would produce urls like "{site}_layouts/15/images".
        ...
        protected string Tokenize(string url, string webUrl, IWeb web = null)
        {
            ...
            if (Uri.TryCreate(webUrl, UriKind.Absolute, out Uri uri))
            {
                ...
                {
                    result = (uri.PathAndQuery.Equals("/") && url.StartsWith(uri.PathAndQuery, StringComparison.Ordinal))
        ...
        /// </summary>
        /// <remarks>
        /// CSOM had <c>Web.IsSubSite()</c>. PnP Core has no equivalent, but comparing the web's
        /// server relative url against the site's is the same test without a round trip - both
        /// properties are already loaded by the time a handler runs.
        /// </remarks>
