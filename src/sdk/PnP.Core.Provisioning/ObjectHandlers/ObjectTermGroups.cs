...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Backlog T10, and the reason for decision D7.</b> Writes go through CSOM because
    /// Graph cannot preserve a caller-supplied term GUID and has no representation for custom sort
    /// order, deprecation, or the shared-versus-local custom property distinction. Templates embed
    /// term GUIDs in taxonomy column defaults and list item values, so a Graph-first path would
    /// silently reassign them. Reads still use PnP Core's <c>ITermStore</c>.</para>
    /// <para><b>The term store is tenant-wide.</b> Unlike every other handler in this wave, what
    /// this one writes is not scoped to the site the template is applied to - a term group created
    /// here is visible to the whole tenant. That is inherent to managed metadata, not a choice, but
    /// it is worth knowing before running a template twice.</para>
    /// </remarks>
    internal class ObjectTermGroups : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// Graph names languages as culture tags (<c>en-US</c>); CSOM and the provisioning schema
        /// use numeric LCIDs. An unrecognised tag yields zero rather than throwing - a label in an
        /// exotic language should not fail the whole extract.
        /// </remarks>
