...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>A single instance is threaded through an entire apply or extract run: every handler
    /// receives it, resolves values through it, and registers new tokens as it creates artefacts.
    /// It is deliberately mutable and deliberately shared.</para>
    /// <para><b>Async construction.</b> Building the token set reads the site - fields, content
    /// types, lists and their views, groups, role definitions, the property bag and more. A
    /// constructor cannot await, so instances are created through
    /// <see cref="CreateAsync(PnPContext, ProvisioningTemplate)"/>.</para>
    /// <para><b>What was preserved deliberately</b> - each of these is easy to "tidy" into a bug:
    /// the three-way cache split, <see cref="ICloneable"/>, the fallback regex that catches tokens
    /// containing braces, and case-insensitive matching throughout.</para>
    /// </remarks>
    public class TokenParser : ICloneable
    {
        ...
        /// </summary>
        /// <remarks>
        /// The lightweight rebase. Used when artefacts have changed underneath the parser but the
        /// target web has not.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// The heavy rebase, used when a hierarchy moves from one site to the next. Only the token
        /// families the incoming template actually references are rebuilt - which is why this takes
        /// the template and not just the context.
        /// </remarks>
        ...
        private void AddResourceTokens(PnPContext context, LocalizationCollection localizations, FileConnectorBase connector)
        {
            ...
            // Multiple resx files per template are supported, so tokens are registered both with
            // and without the resource file name qualifier:
            ...
            foreach (Localization localizationEntry in localizations)
            {
                ...
                if (filePath.EndsWith(".resx", StringComparison.OrdinalIgnoreCase))
                {
                    using (System.IO.Stream stream = connector.GetFileStream(filePath))
                    {
                        ...
                        // Read the resx as raw XML rather than through ResXResourceReader, which
                        // chokes on some files and is not available on all target frameworks.
        ...
        private async Task AddPropertyBagTokensAsync(PnPContext context)
        {
            ...
            // IPropertyValues is an expando model - the key/value pairs hang off Values, not off
            // the model itself, which is why this cannot enumerate AllProperties directly.
        ...
        private async Task AddGroupTokensAsync(PnPContext context)
        {
            ...
            // MIGRATION PHASE 8: PnP Framework additionally registered an {o365groupid:<name>}
            // token per Microsoft 365 group in the tenant, read through UnifiedGroupsUtility with
            // an explicitly acquired Group.Read.All token. That enumeration belongs with the rest
            // of the Microsoft 365 group work in backlog T12 / phase 8; O365GroupIdToken itself is
            // ported and handlers can register instances directly in the meantime.
        }
        ...
        private async Task AddTermStoreTokensAsync(PnPContext context, List<string> tokenIds)
        {
            ...
            try
            {
                if (tokenIds.Contains("termstoreid"))
                {
                    // CSOM enumerated TaxonomySession.TermStores; Graph models exactly one store
                    // per tenant, so there is a single entry to register. Its name is not exposed
                    // by the Graph term store, so the token is registered under the name CSOM used
            ...
            {
                // No term store, or no permission on it. PnP Framework swallowed the same cases;
                // leaving the taxonomy tokens unresolved is better than failing a run whose
                // template may not depend on them after all.
        ...
        private void AddAppPackagesTokens(PnPContext context)
        {
            ...
            // MIGRATION PHASE 8: reading the tenant app catalog needs PnP.Core.Admin's
            // ITenantAppManager, which lands with backlog item T11 / ObjectApplicationLifecycle-
            // Management in phase 8. AppPackageIdToken is ported, so handlers can register
            // instances directly; only the up-front enumeration is deferred.
            _ = context;
        }
        ...
        private async Task AddStorageEntityTokensAsync(PnPContext context)
        {
            try
            {
                // Site collection scoped storage entities live in the root web's property bag,
                // under the same "storageentitiesindex" key the tenant app catalog uses. That part
                // needs no admin rights, so it is resolved here; the tenant-wide set needs the app
                // catalog site and lands with backlog T8 in phase 8.
            ...
            {
                // No property bag access - leave the tokens unresolved, as PnP Framework did.
            }
        }
        ...
        {
            // MIGRATION PHASE 8: site design CRUD is backlog item T7. SiteDesignIdToken is ported;
            // only the tenant-wide enumeration that populates it is deferred.
            _ = context;
        }

        private void AddSiteScriptTokens(PnPContext context)
        {
            // MIGRATION PHASE 8: site script CRUD is backlog item T7, as above.
            _ = context;
        }
        ...
        private async Task AddPageUniqueIdTokensAsync(PnPContext context)
        {
            try
            {
                ...
                // LoadListDataAsStreamAsync pages internally, so unlike CSOM there is no manual
                // ListItemCollectionPosition loop to run here.
            ...
            {
                // No SitePages library (a classic site, for instance) - nothing to register.
            }
        }
        ...
        private async Task RebuildListTokensAsync(PnPContext context, Action<TokenDefinition> addToken)
        {
            ...
            // MIGRATION PHASE 6: PnP Framework additionally registered the root web's lists when
            // provisioning a sub site, so a template could reference a parent-scoped list. That
            // needs a second context against the root web; wire it up with ObjectListInstance,
            // which is the only handler that depends on it.
        }
        ...
        private async Task<string> GetListTitleForMainLanguageAsync(PnPContext context, string name)
        {
            ...
            // MIGRATION PHASE 6: reading a list's title in a specific UI culture needs
            // IUserResource.GetValueForUICultureAsync, which is backlog item T6 and lands at the
            // start of phase 6. Until then this resolves to null, which the caller treats as
            // "no separate main-language title" - the same path a single-language site takes.
            _ = context;
            _ = _webLanguage;
            return null;
        }
