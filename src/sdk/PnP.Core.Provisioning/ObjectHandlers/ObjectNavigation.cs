...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Structural navigation is supported, in both directions and to any depth</b> - but
    /// only writing comes free. PnP Core models the quick launch and the top navigation bar as node
    /// collections and <c>NavigationNodeOptions.ParentNode</c> creates a child, yet
    /// <c>INavigationNode</c> exposes no children collection at all. Reading through the model
    /// alone would silently flatten every nested menu on extract, so children are fetched over REST
    /// - see <c>ReadChildrenAsync</c>.</para>
    /// <para><b>Managed metadata navigation is not.</b> Backlog <b>T9</b>: it is configured through
    /// <c>WebNavigationSettings</c> / <c>TaxonomyNavigation</c>, which live in
    /// <c>Microsoft.SharePoint.Client.Publishing.Navigation</c> and have no REST, Graph or PnP Core
    /// equivalent, and no phase-4 request was built for them. A template asking for it is told so by
    /// name rather than being silently given structural navigation instead - which would look like
    /// success and behave nothing like the template.</para>
    /// <para><b>Nodes are replaced, not merged, when the template says so.</b>
    /// <c>RemoveExistingNodes</c> is honoured by clearing the collection first; without it a
    /// re-apply appends, and the quick launch grows on every run. That is the same failure the
    /// footer handler had in phase 5.</para>
    /// </remarks>
    internal class ObjectNavigation : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// <c>RemoveExistingNodes</c> is set to true on the way out. An extracted template describes
        /// the navigation the site <em>has</em>, and applying it should reproduce that rather than
        /// append to whatever the target already had.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <b>PnP Core's <c>INavigationNode</c> has no children collection</b> - the model is flat,
        /// even though the REST entity is not and <c>NavigationNodeOptions.ParentNode</c> lets you
        /// <em>create</em> a child. Reading through the model alone would silently flatten every
        /// nested menu on extract, so the child collection is fetched directly.
        /// </remarks>
