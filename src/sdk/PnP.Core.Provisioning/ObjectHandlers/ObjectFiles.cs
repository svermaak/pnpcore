...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Extraction is deliberately not implemented</b>, and was not in PnP Framework either:
    /// its <c>ExtractObjects</c> returns the template untouched and its <c>WillExtract</c> returns
    /// false. A file's bytes have to go somewhere, so extracting one is only meaningful with a
    /// connector to write it to - which is a phase-9 concern, and is tracked there rather than
    /// hidden behind a silent no-op here.</para>
    /// <para><b>A NoScript site refuses some uploads outright</b>, by extension and by target
    /// library. Those are skipped with a warning rather than attempted: the failure SharePoint
    /// returns names neither the file nor the reason.</para>
    /// </remarks>
    internal class ObjectFiles : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// All of them can carry executable markup or code. The list matches the one SharePoint
        /// documents for "turn scripting capabilities off".
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Backlog <b>T18</b>. This is what the phase-4 <c>LimitedWebPartManager</c> requests were
        /// built for. A web part already on the page under the same title is left alone, so a
        /// re-apply does not stack duplicates.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// See the note on the class: a file cannot be described without somewhere to put its bytes,
        /// so extraction needs a connector and belongs with the phase-9 round-trip work.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>Written out rather than delegated to <c>IFolder.EnsureFolderAsync</c>.</b> That
        /// method walks from a starting folder and, on a 404, calls <c>AddFolderAsync</c> on the
        /// parent. Started from <c>Web.RootFolder</c> that means asking SharePoint to create a
        /// folder directly under the web, which it refuses with another 404 - so a path whose first
        /// segment is an existing <em>library</em> fails, reporting "Folder not found" about a
        /// folder that is plainly there.</para>
        /// <para>The overwhelmingly common case is that the whole path already exists, because the
        /// library was created by <c>ObjectListInstance</c> earlier in the same run. So the full
        /// path is tried first, and the segment walk is the fallback.</para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Matched by url prefix rather than asked for directly: PnP Core's <c>IFolder</c> has no
        /// navigation to its list, and a folder in <c>_catalogs</c> or at the web root genuinely
        /// has none.
        /// </remarks>
