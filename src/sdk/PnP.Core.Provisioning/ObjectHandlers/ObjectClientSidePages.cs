...
{
    // PnP.Core declares an internal UrlUtility directly in the PnP.Core namespace, which encloses
    // this one - and enclosing-namespace types beat compilation-unit using directives. The alias has
    // to sit inside the namespace body to win; at the top of the file it is silently ignored and
    // Combine binds to the overload that returns a Uri.
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Two passes, and the first one is load bearing.</b> Every page is created as an empty
    /// stub before any of them is filled in. A web part on page A can reference page B by its unique
    /// id through a <c>{pageuniqueid:…}</c> token, and that id only exists once the file does - so
    /// without the stub pass, a template with cross-page links applies with dead references and no
    /// error.</para>
    /// <para><b>The page layout is set during the stub pass on purpose.</b> SharePoint will not let
    /// a page's layout type change after creation, so it has to be right the first time.</para>
    /// <para><b>Extraction lives elsewhere</b> - see
    /// <see cref="ObjectClientSidePageContents"/>. This handler's <c>WillExtract</c> is
    /// <c>false</c>, as it was in PnP Framework.</para>
    /// </remarks>
    internal class ObjectClientSidePages : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// <b>A schema workaround, not a design.</b> Topic page header controls cannot be positioned
        /// on the page and do not belong to any section, but the schema has nowhere else to put them
        /// - so extraction writes them into a one-column section with this sentinel order, and this
        /// handler skips that order when laying sections out and reads it back separately. Both
        /// sides carry this comment so the two are changed together.
        /// </remarks>
        ...
        {
            // Reading is ObjectClientSidePageContents' job - see the remarks on the class.
        ...
        public override async Task<TokenParser> ProvisionObjectsAsync(PnPContext context, ProvisioningTemplate template, TokenParser parser, ApplyConfiguration configuration)
        {
            using (context.Logger?.BeginScope(Name))
            {
                ...
                // Pass 1 - create every page as an empty stub, so its unique id token exists before
                // any web part can reference it. See the remarks on the class.
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>The source language must already match.</b> A site has exactly one source
        /// language and it cannot be changed, so applying a template whose pages were authored in a
        /// different one would silently produce translations of the wrong original. That is a hard
        /// failure rather than a warning.</para>
        /// <para>Adding a supported language goes through the CSOM request built for
        /// <see cref="ObjectSupportedUILanguages"/> (backlog T2) - REST accepts the call and does
        /// nothing.</para>
        /// </remarks>
        ...
        private async Task EnableFeatureAsync(PnPContext context, Guid featureId, string description)
        {
            ...
            {
                // Unlike ObjectFeatures, this is fatal: the pages this template describes cannot
                // exist without the feature, so continuing would produce a half-applied site.
        ...
        private async Task CreateTranslationsAsync(PnPContext context, TokenParser parser, ClientSidePageModel clientSidePage, List<string> preCreatedPages)
        {
            ...
            // A generation run reports every translation that exists, not only the new ones - so
            // when nothing was generated the previously available set is the right answer.
            ...
            ClientSidePageModel clientSidePage, int currentPageIndex)
        {
            ...
            // The layout has to be set now - SharePoint refuses to change it after creation.
            ...
            BaseClientSidePage clientSidePage, List<string> preCreatedPages, int currentPageIndex, int totalPages)
        {
            ...
            // A repost page is a pointer to content elsewhere; it has no canvas of its own.
        ...
        private void ApplyHeader(IPage page, BaseClientSidePage clientSidePage, TokenParser parser)
        {
            ...
            switch (clientSidePage.Header.Type)
            {
                ...
                case ClientSidePageHeaderType.Default:
                    // The PageTitle web part header is not a schema type, so extraction records it
                    // as Default. It is re-derived here from the presence of a PageTitle web part -
                    // which can sit in any section, not just the first.
        ...
        private async Task ApplySectionsAsync(PnPContext context, IPage page, BaseClientSidePage clientSidePage, TokenParser parser)
        {
            ...
            // A page with no sections still needs somewhere to put its controls.
            ...
            {
                // The topic header control section is not a real section - see the remarks on
                // TopicHeaderControlSectionOrder.
                ...
                // A column of 0 or less would index outside the section; it means "the first column".
        ...
        {
            // Column numbers are 1 based in the template and 0 based on the page.
            ...
            // Rewrite ids that only make sense on the source site - today, the List web part's list id.
            ...
            if (!string.IsNullOrEmpty(control.JsonControlData))
            {
                ...
                if (webPart == null)
                {
                    ...
                    if (webPart == null)
                    {
                        // An empty section control: the section already creates one when it has no
                        // web parts, so adding another would duplicate it.
                        return;
                    }
                }
        ...
        private static void AddTextControl(IPage page, CanvasSectionModel section, ICanvasColumn column, CanvasControlModel control, TokenParser parser)
        {
            ...
            {
                // Older templates stored the text as the single value of a flat dictionary rather
                // than in ControlProperties.
        ...
        /// </summary>
        /// <remarks>
        /// A third party web part is identified by name or by id; a first party one by the well
        /// known id its <see cref="WebPartType"/> maps to. Returning <c>null</c> is normal and not an
        /// error - the caller then builds a generic web part straight from the stored JSON, which is
        /// how a page survives being applied to a site where the component is not installed.
        /// </remarks>
        private static IPageComponent ResolveComponent(IPage page, CanvasControlModel control, IEnumerable<IPageComponent> availableComponents)
        {
            if (control.Type == WebPartType.Custom)
            {
                ...
                {
                    // Component ids are stored braced by some sources and bare by others.
        ...
        /// </summary>
        /// <remarks>
        /// <c>WebPartId</c> has no public setter - it is derived from the component a web part was
        /// built from, and there is no component here. Setting it through reflection is what PnP
        /// Framework did and it is still the only way; without it the web part is written with an
        /// empty id and the page renders a placeholder.
        /// </remarks>
        ...
        private async Task ApplyTopicHeaderControlsAsync(PnPContext context, IPage page, BaseClientSidePage clientSidePage, TokenParser parser)
        {
            ...
            // A topic page still needs a canvas, even when everything visible is a header control.
            ...
            TokenParser parser, string url, bool existed)
        {
            ...
            {
                // A choice field with exactly one possible value today.
            ...
            if (dirty)
            {
                // SystemUpdate on an existing page so re-applying a template does not bump every
                // page's version and modified-by; a brand new page has no history to preserve.
                if (existed)
                {
        ...
        /// </summary>
        /// <remarks>
        /// The template carries a <em>parent</em> content type id, because the leaf id is generated
        /// per site. The match is therefore the longest installed id that the template's id is a
        /// prefix of - which is what CSOM's <c>BestMatchContentTypeId</c> did.
        /// </remarks>
        ...
        private static async Task ApplyPublishingAndCommentsAsync(IPage page, BaseClientSidePage clientSidePage)
        {
            ...
            {
                // The home page is always the home page, and a template is not news.
                ...
                // A repost page shows someone else's content, so it has no comments of its own.
                if (page.LayoutType != PageLayoutType.RepostPage)
                {
        ...
        private void WarnOnUnsupportedFieldValues(PnPContext context, BaseClientSidePage clientSidePage, TokenParser parser)
        {
            ...
            // MIGRATION PHASE 6: writing arbitrary typed field values needs ListItemUtilities, which
            // lands with ObjectListInstanceDataRows. Reported rather than skipped quietly - a topic
            // page's _EntityId and a Spaces page's SpaceContent both travel this way, and losing
            // them produces a page that looks right and behaves wrong.
        ...
        private void WarnOnUnsupportedSecurity(PnPContext context, BaseClientSidePage clientSidePage, string pageName)
        {
            ...
            // MIGRATION PHASE 6: unique page permissions need the role assignment plumbing
            // ObjectSiteSecurity owns.
        ...
        {
            // A translated page always lives in the library root, even when its source is a template.
        ...
        /// </summary>
        /// <remarks>
        /// Only an Article page can be a page template; the flag is ignored on every other layout.
        /// </remarks>
        ...
        private static string DeterminePageName(TokenParser parser, BaseClientSidePage clientSidePage)
        {
            ...
            {
                // A page template lives in the templates folder, so any folder in the name is dropped.
