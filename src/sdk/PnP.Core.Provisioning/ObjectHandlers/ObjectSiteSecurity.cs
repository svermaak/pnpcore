...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Backlog T3 lives here.</b> The three associated-group properties are get-only in
    /// PnP Core and have no REST equivalent, so assigning one goes through
    /// <see cref="SetAssociatedGroupRequest"/>. The same is true of a site collection
    /// administrator: <c>ISharePointUser.IsSiteAdmin</c> is get-only.</para>
    /// <para><b>Groups are created in dependency order.</b> A group can be owned by another group,
    /// and SharePoint will not accept an owner that does not exist yet, so the template's groups are
    /// sorted so that an owner comes before the group it owns.</para>
    /// <para><b>Nothing is applied on a NoScript site's associated groups.</b> SharePoint refuses
    /// the change, and attempting it produces an error naming neither the group nor the reason.</para>
    /// </remarks>
    internal class ObjectSiteSecurity : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// A topological sort rather than a comparison sort: "A owns B" is a dependency, not an
        /// ordering key, and a comparison sort has no way to express it. A cycle - which the schema
        /// permits and SharePoint does not - is broken by emitting the remaining groups in template
        ...
        /// </summary>
        /// <remarks>
        /// The current user is left alone. Removing one's own administrator rights mid-run makes
        /// every subsequent handler fail with access denied, and the template almost never means it.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <b>An associated group is exported for its settings but not for its members.</b> Its
        /// members are already in <c>AdditionalOwners</c> / <c>AdditionalMembers</c> /
        /// <c>AdditionalVisitors</c>, and listing them again would have the apply add every one of
        /// them twice. Dropping the group entirely is not an option either - that loses its
        /// description and its membership settings.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Groups first because a group title and a user's display name can collide, and a template
        /// naming a group means the group. A numeric value is a principal id already.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// A web without an associated group of a given kind returns an object whose properties are
        /// unset, and touching one throws rather than returning null.
        /// </remarks>
