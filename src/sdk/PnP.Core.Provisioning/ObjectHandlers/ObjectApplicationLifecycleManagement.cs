...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T14 area</b>, phase 8. <c>PnP.Core.Admin</c> covers this almost entirely:
    /// <c>ITenantAppManager</c> and <c>ISiteCollectionAppManager</c> between them expose add, deploy,
    /// retract, remove, install, upgrade and uninstall, which is the whole of what the handler
    /// needs.</para>
    /// <para><b>Two catalogs, two jobs.</b> <c>&lt;pnp:Apps&gt;</c> installs an app that is already
    /// in a catalog <em>into this site</em>; <c>&lt;pnp:AppCatalog&gt;</c> puts a package <em>into
    /// the site collection's own catalog</em>. They are separate operations against separate APIs and
    /// the template can ask for either.</para>
    /// <para><b>This does more than PnP Framework.</b> PnP Framework leaves the
    /// <c>&lt;pnp:AppCatalog&gt;</c> half as an explicit no-op - "the ALM API do not support the
    /// local Site Collection App Catalog" - while still counting its packages in
    /// <c>WillProvision</c>. That combination is the worst of both: the engine announces it will
    /// provision the element and then silently does nothing with it. PnP Core does support the site
    /// collection catalog, so the packages are provisioned here rather than dropped.</para>
    /// <para><b>Extraction lists installed apps by id and nothing else.</b> A template that named the
    /// package file would only be applicable on a tenant that already had that file; the id is what
    /// travels. This matches PnP Framework.</para>
    /// </remarks>
    internal class ObjectApplicationLifecycleManagement : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// <c>SyncMode.Synchronously</c> in a template means "do not move on until this app is really
        /// installed", because a later handler may depend on the columns or web parts it brings. Five
        /// minutes matches PnP Framework.
        /// </remarks>
        ...
        {
            // The subsite test is deliberately not made here. It needs the web loaded, which has not
            // happened yet, so asking now answers "root web" for every site - see IsSubSiteAsync.
            ...
            TokenParser parser, ApplyConfiguration configuration)
        {
            ...
            // An app catalog belongs to a site collection, and installing an app is a site collection
            // operation. On a subsite both are somebody else's business.
            ...
            Model.ApplicationLifecycleManagement alm, TokenParser parser)
        {
            ...
            try
            {
                // Creating the catalog rather than requiring one: a template that ships packages is
                // asking for a catalog, and the alternative is failing on a site that has simply
                // never had one.
            ...
            ISiteCollectionAppManager manager, PackageModel package, TokenParser parser)
        {
            ...
            {
                // Publish on its own carries the id; upload-and-publish uses the id the upload
                // returned, which is not the same as the product id in the template.
        ...
        private async Task ApplyAppsAsync(PnPContext context, Model.ApplicationLifecycleManagement alm, TokenParser parser)
        {
            ...
            // Read once. Installed state is what every branch below turns on, and asking per app
        ...
        /// </summary>
        /// <remarks>
        /// <b>Installed is not the same as up to date.</b> An upgrade leaves the app installed
        /// throughout, so waiting for "installed" would return immediately - the test that means
        /// something is that the installed version has caught up with the catalog version. Uninstall
        /// is the mirror image: wait for the installed version to disappear.
        /// </remarks>
            ...
            Guid appId, AppAction action)
        {
            ...
            while (true)
            {
                ...
                {
                    // Thrown, not warned: the template said "synchronously", which means a later
                    // handler is expected to be able to rely on this app being ready.
        ...
        /// </summary>
        /// <remarks>
        /// <c>GetAvailableAsync()</c> returns everything the catalog offers; <c>InstalledVersion</c>
        /// is what distinguishes "available to this site" from "installed in it".
        /// </remarks>
