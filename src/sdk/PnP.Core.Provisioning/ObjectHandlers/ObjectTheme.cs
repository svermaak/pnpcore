...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Apply only.</b> PnP Framework's <c>ExtractObjects</c> returns the template untouched
    /// and <c>WillExtract</c> returns <c>false</c> — themes are never extracted, only applied. That
    /// is preserved rather than "improved": a site's effective theme is a merge of tenant themes and
    /// local overrides, and there is no faithful way to reduce it back to the single
    /// <c>&lt;pnp:Theme&gt;</c> element the schema allows.</para>
    /// <para>Three cases, in the order PnP Framework tried them: a built-in theme by name, an inline
    /// palette, then a tenant-defined theme by name.</para>
    /// </remarks>
