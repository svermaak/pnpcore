...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    internal partial class ObjectTeams
    {
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>Owners are applied before members, and the difference matters.</b> Graph refuses
        /// to remove the last owner of a group, so a template that swaps the whole owner list has to
        /// add the new ones first - which is what happens naturally by adding before removing.</para>
        /// <para><b>Removal only happens when the template asks for it.</b> <c>ClearExistingOwners</c>
        /// and <c>ClearExistingMembers</c> are opt-in: a template that lists two owners is normally
        /// saying "make sure these two are owners", not "these two and nobody else".</para>
        /// </remarks>
            ...
            IEnumerable<TeamSecurityUserModel> wanted, bool clearExisting, TokenParser parser)
        {
            ...
            foreach (TeamSecurityUserModel user in declared)
            {
                ...
                try
                {
                    // Added by reference, which is how Graph models a directory relationship - the
                    // body is a url to the user, not the user.
            ...
            string collection)
        {
            ...
            {
                // An unreadable membership list is reported by the caller's add attempts failing,
                // which say more than "could not read the list" would.
            }
        ...
        /// </summary>
        /// <remarks>
        /// <b>A group directory setting, not a team setting.</b> It lives in the group's
        /// <c>settings</c> collection under the <c>Group.Unified.Guest</c> template, has to be
        /// created the first time and patched thereafter - the same shape as the tenant's
        /// <c>Group.Unified</c> settings in <c>ObjectTenant</c>.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para>One of spike <b>S2</b>'s six gaps: <c>ITeamAppCollection</c> is read-only, so
        /// installing is a Graph call.</para>
        /// <para><b>Already installed is not a failure.</b> Graph answers a conflict, and a
        /// re-applied template hits it every time - so it is checked first rather than caught.</para>
        /// </remarks>
        ...
        private static async Task<HashSet<string>> InstalledAppIdsAsync(PnPContext context, string teamId)
        {
            ...
            {
                // Treated as "none installed": the add below reports its own conflict if it is wrong.
            }
            ...
            TeamChannelModel channel, TokenParser parser)
        {
            ...
            // A private channel needs a membership type and at least one owner, and the owner has to
            // be supplied at creation - it cannot be added afterwards.
        ...
        private static async Task<Dictionary<string, string>> ReadChannelsAsync(PnPContext context, string teamId)
        {
            ...
            {
                // Empty means "create them all", and a create that collides reports itself.
            }
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>Built generically, not through PnP Core's typed tab helpers.</b> Spike S2 counted
        /// thirteen of them - <c>AddWikiTab</c>, <c>AddPlannerTab</c> and so on - and they are better
        /// than this for code that knows which kind of tab it wants. A provisioning template does not:
        /// the schema expresses a tab as an app id plus a configuration bag, exactly as Graph does, so
        /// mapping it onto a typed helper would mean a lookup table from app id to method that could
        /// only ever cover the tabs Microsoft shipped when it was written.</para>
        /// </remarks>
            ...
            TeamChannelModel channel, TokenParser parser)
        {
            ...
            foreach (TeamTabModel tab in channel.Tabs)
            {
                ...
                try
                {
                    ...
                    {
                        // The app the tab is an instance of, bound by reference - a tab cannot be
                        // created without it.
            ...
            string channelId)
        {
            ...
            {
                // As for channels: empty means "add them all".
            }
        ...
        /// </summary>
        /// <remarks>
        /// <b>Posted every time, and that is deliberate.</b> A message has no identity a template can
        /// match on, so there is no way to tell "already posted" from "posted again" - and Graph
        /// offers no way to delete one. Re-applying a template with messages will repeat them, which
        /// is PnP Framework's behaviour too.
        /// </remarks>
        ...
        private static IEnumerable<JsonElement> ArrayOf(string json)
        {
            ...
            using (JsonDocument document = JsonDocument.Parse(json))
            {
                ...
                foreach (JsonElement item in value.EnumerateArray())
                {
                    // Cloned: the document is disposed when this iterator ends, and a JsonElement is
                    // a view over it.
