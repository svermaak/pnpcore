...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    internal partial class ObjectListInstance
    {
        ...
        /// </summary>
        /// <remarks>
        /// A <c>&lt;FieldRef&gt;</c> is not a column definition, it is a pointer at a site column
        /// plus three list-level overrides. The column itself belongs to <c>ObjectField</c>, which
        /// is why a reference to a column defined in this same template is skipped until the step
        /// that column belongs to comes round.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <c>RelationshipDeleteBehavior</c> is not accepted on the create call - it has to be set
        /// afterwards - and the restrict and cascade behaviours additionally require the column to
        /// be indexed. Sending the attribute as-is fails; dropping it silently loses the behaviour,
        /// which is what PnP Framework chose and what is preserved here.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// A folder named <c>/</c> is not a folder: it is how the schema addresses the list root, so
        /// that a template can put property bag entries and child folders on the root itself.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para>The two are separate features with separate columns, and the template has a flag
        /// for each. <c>IList.EnableAudienceTargetingAsync</c> adds only the modern pair
        /// (<c>_ModernAudienceTargetUserField</c> and <c>_ModernAudienceAadObjectIds</c>) - it has
        /// nothing to do with the classic <c>Target_x0020_Audiences</c> column, so that one is added
        /// here from its own schema, as PnP Framework did.</para>
        /// <para>The classic column's id is fixed across every site; it is part of the field
        /// definition rather than something generated per list.</para>
        /// </remarks>
