...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>This handler runs three times</b>, once per
    /// <see cref="FieldAndListProvisioningStepHelper.Step"/>. That is not an optimisation, it is
    /// the only way a template with cross-list lookups can work: a lookup column cannot be created
    /// until the list it points at exists, so lists and their plain columns are made first
    /// (<c>ListAndStandardFields</c>), lookups second (<c>LookupFields</c>), and everything that
    /// might reference a column - views, default values, folders - last (<c>ListSettings</c>).
    /// The create-or-update pass runs in all three; only the settings block is gated.</para>
    /// <para><b>List creation goes through CSOM.</b> <c>IListCollection.AddAsync</c> takes a title
    /// and a template type, and derives the url from the title. A template sets the url
    /// independently, and the url is the list's real identity - <c>{listurl:…}</c> tokens and
    /// lookup columns resolve against it, and it does not change when the list is renamed. See
    /// <see cref="CreateListRequest"/>.</para>
    /// <para><b>The parser is cloned per list.</b> A list's own columns register tokens that must
    /// be visible to that list's other elements without leaking into the next list, where the same
    /// display name may mean a different column. The list id and url tokens do go into the shared
    /// parser - those are global facts.</para>
    /// </remarks>
    internal partial class ObjectListInstance : ObjectHandlerBase
    {
            ...
            TokenParser parser, ApplyConfiguration configuration)
        {
            ...
            {
                // A clone, so this list's column tokens do not leak into the next list. Tokens that
                // are genuinely global - the list's id and url - are added to both.
                ...
                await CheckContentTypesAsync(context, template, templateList).ConfigureAwait(false);
                ...
                if (existing == null)
                {
                    ...
                    // Referenced again later in the same template - for example a list validation
                    // formula pointing at a column of the list created first - it is an update from
                    // here on, not a second create.
            ...
            if (step == FieldAndListProvisioningStepHelper.Step.ListSettings)
            {
                ...
                // After the folders exist - a default column value is scoped to a folder path, and
                // setting one for a folder that has not been created yet is silently dropped.
                foreach (ListInfo listInfo in processedLists)
                {
                ...
                // Last: creating or replacing a view can drop the list off the quick launch.
                foreach (ListInfo listInfo in processedLists)
                {
            ...
            TokenParser parser, bool isNoScriptSite)
        {
            ...
            {
                // The site assets library has to come from the built-in creation path. One created
                // as a plain document library at the same url differs in ways that break the modern
                // page infrastructure that stores its images there.
            ...
            TokenParser parser, string webUrl, bool isNoScriptSite)
        {
            ...
            // The list came from the web's collection, which was loaded with only the few properties
            // the match needed. Everything the update path touches has to be loaded before it can be
            // read, and PnP Core materialises a collection's items once - so this is a fresh read.
            ...
            // A url change is a folder move, not a property write - the url is not settable.
            string wantedUrl = UrlUtility.Combine(webUrl, templateList.Url);
        ...
        /// </summary>
        /// <remarks>
        /// Several settings are refused by particular list templates rather than ignored - setting
        /// <c>EnableAttachments</c> on a document library or versioning on a survey throws - so the
        /// exclusions here are load bearing, not defensive.
        /// </remarks>
            ...
            TokenParser parser, bool isNew)
        {
            ...
            // 0 means "not specified" in the schema, and 1 is the server's default. Sending 0 turns
            // read security into an invalid value rather than leaving it alone.
            ...
            // EnableAttachments does not apply to a survey, a document library or a picture library,
            // and setting it on one of those throws rather than being ignored. 850 is the pages
            // library on a publishing site, which behaves like a document library here.
        ...
        private static bool ApplyVersioning(CoreList list, ListInstance templateList)
        {
            ...
            // Approver-only drafts require moderation; asking for it without moderation is rejected.
            ...
            {
                // Only meaningful with minor versions on, and setting it to zero throws.
        ...
        /// </summary>
        /// <remarks>
        /// <b>PnP Core models these read-only.</b> <c>IInformationRightsManagementSettings</c>
        /// implements <c>IDataModel</c> but not <c>IDataModelUpdate</c>, so the property setters
        /// stage a change that has no way to be sent. The REST endpoint does accept a MERGE, so
        /// that is what this does - the alternative is dropping the whole
        /// <c>&lt;pnp:IRMSettings&gt;</c> element on the floor.
        /// </remarks>
        private static async Task WriteIrmSettingsAsync(PnPContext context, CoreList list, ListInstance templateList, TokenParser parser)
        {
            ...
            {
                // The schema carries a relative span; the endpoint wants an absolute date.
        ...
        /// </summary>
        /// <remarks>
        /// Without this the binding silently does nothing and the list ends up with the wrong
        /// columns - a failure that shows up much later and looks like a different bug.
        /// </remarks>
        private static async Task CheckContentTypesAsync(PnPContext context, ProvisioningTemplate template, ListInstance templateList)
        {
            ...
            List<string> available = context.Web.AvailableContentTypes.AsRequested().Select(ct => ct.StringId).ToList();

            foreach (ContentTypeBinding binding in templateList.ContentTypeBindings)
            {
                bool found = template.ContentTypes.Any(t => string.Equals(t.Id, binding.ContentTypeId, StringComparison.OrdinalIgnoreCase))
                    || available.Any(id => string.Equals(id, binding.ContentTypeId, StringComparison.OrdinalIgnoreCase));

                if (!found)
                {
                    throw new Exception($"Referenced content type {binding.ContentTypeId} is available neither in the site nor in the template.");
                }
            }
        }
            ...
            bool isNewList, TokenParser parser)
        {
            ...
            foreach (ContentTypeBinding binding in templateList.ContentTypeBindings)
            {
                ...
                {
                    // It was already on the list, so it is not one of the ones to sweep away.
                    toRemove.Remove(onList);
                }
        ...
        /// </summary>
        /// <remarks>
        /// SharePoint has no "is default" flag and no "hide in the new button" flag. The list's
        /// <em>unique content type order</em> carries both: the first entry is the default, and an
        /// entry that is absent from the order is hidden. So all three template attributes turn into
        /// a single ordered list here.
        /// </remarks>
            ...
            List<IContentType> visible, List<IContentType> hidden)
        {
            ...
            if (order.Count == 0)
            {
                // Everything is hidden. An empty order is not accepted, and would leave the list
                // with no way to create an item, so the default stays whatever the server chose.
                return;
            }
        ...
        /// </summary>
        /// <remarks>
        /// A list content type's id is the site content type's id with a suffix, so this is a
        /// longest-prefix match rather than an equality test.
        /// </remarks>
            ...
            TokenParser parser, bool isNoScriptSite)
        {
            ...
            foreach (CustomAction customAction in templateList.UserCustomActions)
            {
                ...
                {
                    // The registration is forced to this list while updating, then blanked again:
                    // leaving it on the model would write a list-scoped registration back into the
                    // template on the next extract.
        ...
        private async Task ApplyWebhooksAsync(PnPContext context, CoreList list, ListInstance templateList, TokenParser parser)
        {
            ...
            {
                // App-only contexts cannot read subscriptions. That is a reason to skip webhooks,
                // not a reason to fail the whole template.
            ...
            foreach (Webhook webhook in templateList.Webhooks)
            {
                ...
                {
                    // An extracted template carries the remaining lifetime, so a stale export has a
                    // zero or negative span. Re-adding it would immediately expire.
                ...
                {
                    // A webhook endpoint that does not answer the validation handshake fails here.
                    // An exported template routinely references one, and that must not stop the run.
        ...
        /// </summary>
        /// <remarks>
        /// An absent element in the schema deserializes to an empty string, which is
        /// indistinguishable from "set it to empty". Treating empty as "leave alone" matches what
        /// the engine has always done, and avoids blanking a document template or form url that the
        /// template simply did not mention.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// The title is registered per supported UI language too: a localized template refers to a
        /// list by whichever title the reader's language shows, and <c>{listid:…}</c> has to resolve
        /// for all of them.
        /// </remarks>
