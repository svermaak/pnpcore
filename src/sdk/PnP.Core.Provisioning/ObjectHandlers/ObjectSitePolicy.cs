...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T22</b>. There is no REST or Graph surface for site policies at all; the
    /// CSOM <c>ProjectPolicy</c> type is the only way to read or set one, which is why the three
    /// requests were built in phase 4.</para>
    /// <para><b>A policy is applied by object, not by name.</b> <c>ApplyProjectPolicy</c> takes the
    /// policy itself, so the template's name has to be matched against the site's available
    /// policies first and the position passed on - see <see cref="ApplyProjectPolicyRequest"/>.</para>
    /// <para><b>A missing policy warns rather than fails.</b> Policies are defined in the Content
    /// Type Hub and published to sites, so a template that names one is describing a tenant
    /// arrangement it cannot itself create.</para>
    /// </remarks>
