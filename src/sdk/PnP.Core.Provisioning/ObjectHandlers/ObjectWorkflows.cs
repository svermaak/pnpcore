...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T16</b>. The workflow services are CSOM only, and the eight requests were
    /// built in phase 4.</para>
    /// <para><b>SharePoint 2013 workflows are retired.</b> On a tenant where the service is off -
    /// which is now most of them - <c>WorkflowServicesManager</c> returns null and every call fails
    /// with "Cannot invoke method or retrieve property from null object". That is a tenant state,
    /// not a fault, so the handler reports it once and stops rather than failing a template that
    /// merely happens to carry a workflow element.</para>
    /// <para><b>A definition is published, then subscribed.</b> A subscription references its
    /// definition by id, so the order is fixed: save and publish every definition first, and only
    /// then bind the subscriptions - which is why they are two passes rather than one loop.</para>
    /// </remarks>
    internal class ObjectWorkflows : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// The XAML is the workflow. A definition whose file is missing cannot be provisioned at
        /// all, and creating an empty one would produce a workflow that silently does nothing.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// The schema's <c>ListId</c> is a string because it may be a <c>{listid:…}</c> token rather
        /// than a literal. A site-scoped subscription has none.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Probing once and reporting once is the point. Without this the handler emits the same
        /// "Cannot invoke method or retrieve property from null object" for every definition and
        /// every subscription, which buries the one fact that matters: the service is gone.
        /// </remarks>
