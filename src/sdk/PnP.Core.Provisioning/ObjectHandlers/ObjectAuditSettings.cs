...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Two of the three settings are REST and one is not.</b> <c>ISite</c> exposes
    /// <c>AuditLogTrimmingRetention</c> and <c>TrimAuditLog</c> as ordinary settable properties, but
    /// <c>ISite.Audit</c> is <em>commented out</em> in PnP Core - so the audit flags themselves need
    /// CSOM. Backlog <b>T21</b>; the requests were built in phase 4.</para>
    /// <para><b>Site collection scope only.</b> Auditing is configured on the site collection, so a
    /// subsite has nothing to apply and nothing to report.</para>
    /// <para><b>Retention cannot be set on a NoScript site.</b> SharePoint refuses it, so the
    /// handler skips that one property with a warning and still applies the other two, which is
    /// what PnP Framework does.</para>
    /// </remarks>
    internal class ObjectAuditSettings : ObjectHandlerBase
    {
        ...
        {
            // Deliberately not asking whether this is a subsite here. WillProvision is called to
            // size the progress bar, before the engine has necessarily loaded the web, so the
            // answer would be a guess. The real check is in the async paths, which load first.
            ...
            TokenParser parser, ApplyConfiguration configuration)
        {
            ...
            // Loaded before asking, rather than relying on the sync best-effort answer: that one
            // returns false when it cannot tell, which would have a subsite try to write site
            // collection settings.
        ...
        /// </summary>
        /// <remarks>
        /// <c>ISite.Audit</c> is commented out in PnP Core, so there is no model to set. The commit
        /// is <c>Audit.Update()</c> - on the audit object, not on the site - which is the same trap
        /// as regional settings and the term store's <c>CommitAll</c>.
        /// </remarks>
            ...
            ExtractConfiguration configuration)
        {
            ...
            // Nothing differs from the defaults, so there is nothing worth writing. An element that
            // just restates the defaults makes every extracted template noisier for no gain, and
            // PnP Framework applied the same rule.
            if (!anythingSet)
            {
                return template;
            }
