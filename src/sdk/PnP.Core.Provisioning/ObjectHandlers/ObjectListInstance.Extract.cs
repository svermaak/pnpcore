    ...
    internal partial class ObjectListInstance
    {
            ...
            ExtractConfiguration configuration)
        {
            ...
            foreach (CoreList siteList in toProcess)
            {
                ...
                ListInstance list = await ExtractListAsync(context, template, creationInfo, siteList, allLists, webUrl)
            ...
            ProvisioningTemplateCreationInformation creationInfo, CoreList siteList, List<CoreList> allLists, string webUrl)
        {
            ...
            await ExtractContentTypesAsync(context, siteList, list, contentTypeFields).ConfigureAwait(false);
            ...
            List<FieldRef> contentTypeFields)
        {
            ...
            {
                // A list content type's id is the site content type's id plus a suffix; the template
                // binds to the site one, which is what the id without that suffix names.
                string parentId = ParentContentTypeId(contentType.StringId);
        ...
        /// <summary>
        /// A list content type's id with the list-specific suffix removed.
        /// </summary>
        /// <remarks>
        /// The suffix is <c>00</c> followed by a 32 character hex guid, appended to the parent's id.
        /// An id that does not carry one is its own parent - a content type defined on the list.
        /// </remarks>
        private static string ParentContentTypeId(string listContentTypeId)
        {
            const int suffixLength = 34;

            if (listContentTypeId != null && listContentTypeId.Length > suffixLength
                && listContentTypeId.Substring(listContentTypeId.Length - suffixLength, 2) == "00")
            {
                return listContentTypeId.Substring(0, listContentTypeId.Length - suffixLength);
            }

            return listContentTypeId;
        }
        ...
        /// </summary>
        /// <remarks>
        /// A column that exists unchanged as a site column is written as a <c>&lt;FieldRef&gt;</c>;
        /// one that does not, or that the list has customised, is written out in full. Getting this
        /// the wrong way round produces a template that either creates duplicate site columns or
        /// loses per-list customisations.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// A selection entry matches on title, on id, or on the tail of the url - because a caller
        /// naming "Documents" means the library at <c>/Shared Documents</c> as readily as the one
        /// titled "Documents".
        /// </remarks>
