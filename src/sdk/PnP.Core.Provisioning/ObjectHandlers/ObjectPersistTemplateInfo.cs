...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>The write side of the pair completed by <see cref="ObjectRetrieveTemplateInfo"/>. It
    /// runs last in an apply, so <c>ProvisioningTime</c> and <c>Result</c> describe a run that
    /// actually finished.</para>
    /// <para><b>NoScript sites get nothing.</b> Property bag writes are refused there, so the
    /// handler returns early rather than letting two writes fail in turn - the same choice
    /// <see cref="ObjectPropertyBagEntry"/> makes, for the same reason.</para>
    /// </remarks>
