...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// The first wave-1 handler, and the smallest - chosen to validate the phase 3 base-class design
    /// against something real before the larger handlers commit to it.
    /// </remarks>
    internal class ObjectSiteSettings : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// There is no API for this on either SDK - PnP Framework read and wrote the same key.
        /// It lives on the <b>root web</b>, not the site.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Two identically named enums - PnP Core's <c>PnP.Core.Model.SharePoint.SearchBoxInNavBar</c>
        /// and the template model's - so the conversion is explicit and by name. PnP Framework used
        /// <c>Enum.Parse</c> on <c>ToString()</c>, which throws at run time on any member the two do
        /// not share; a switch fails to compile instead if either enum gains a member.
        /// </remarks>
