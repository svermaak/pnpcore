...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    public static class WebhookSender
    {
        ...
        private static async Task PostAsync(HttpClient httpClient, string url, ProvisioningTemplateWebhookBodyFormat bodyFormat, Dictionary<string, string> requestParameters)
        {
            switch (bodyFormat)
            {
                ...
                case ProvisioningTemplateWebhookBodyFormat.Xml:
                    ...
                    {
                        // PnP Framework's synchronous branch dropped this task on the floor, so an
                        // XML bodied webhook could be abandoned mid-flight when the run finished.
                        // Awaiting it here makes the sync and async branches behave as documented.
                        await httpClient.PostAsync(url, stringContent).ConfigureAwait(false);
                    }
                    break;
        ...
        /// </summary>
        /// <remarks>
        /// <para>PnP Framework handed the parameter dictionary to <see cref="XmlSerializer"/>,
        /// which cannot serialize a type implementing <c>IDictionary</c> - so
        /// <see cref="ProvisioningTemplateWebhookBodyFormat.Xml"/> threw
        /// <see cref="NotSupportedException"/> every time it was used, on every version of .NET.
        /// The exception escaped too: the surrounding <c>catch</c> only handled
        /// <see cref="HttpRequestException"/>, so an XML webhook failed the whole provisioning run
        /// rather than just the notification.</para>
        /// <para>Since the format never produced output, there is no wire contract to preserve.
        /// The shape below mirrors the JSON and form-encoded bodies - a flat set of key/value
        /// pairs - and <see cref="XElement"/> handles escaping.</para>
        /// </remarks>
