...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Ordered by id, and that is deliberate.</b> A content type id encodes its ancestry -
    /// <c>0x0100AB</c> is a child of <c>0x0100</c> - so sorting by id puts every parent before its
    /// children. A template can therefore define a whole hierarchy in one go without stating the
    /// order itself.</para>
    /// <para><b>It runs in both field passes.</b> A content type's column links can only be created
    /// once the columns exist, and a lookup column is not created until the second pass - so each
    /// field ref is added in the pass that matches the column it points at.</para>
    /// <para><b><c>ReadOnly</c> is set last.</b> A read-only content type cannot be updated, so
    /// setting it during the first pass would lock out every change the second pass needs to make.
    /// </para>
    /// </remarks>
    internal class ObjectContentType : ObjectHandlerBase
    {
        ...
        public override async Task<TokenParser> ProvisionObjectsAsync(PnPContext context, ProvisioningTemplate template, TokenParser parser, ApplyConfiguration configuration)
        {
            using (context.Logger?.BeginScope(Name))
            {
                ...
                foreach (ContentTypeModel contentType in template.ContentTypes.OrderBy(ct => ct.Id, StringComparer.OrdinalIgnoreCase))
                {
                    ...
                    {
                        string message = $"The content type '{name}' ({contentType.Id}) is sealed or read only on this site, so it was not updated.";
                        context.Logger?.LogWarning("{Source}: {Message}", Constants.LOGGING_SOURCE, message);
                        WriteMessage(message, ProvisioningMessageType.Warning);
                    }
        ...
        /// </summary>
        /// <remarks>
        /// One query, because PnP Core materialises a collection's items once - asking a single
        /// content type for a property later leaves it unloaded and throws on read.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <b>The loaded collection is not authoritative for a content type created moments ago.</b>
        /// The second provisioning pass would otherwise not find what the first pass created,
        /// create it again, and get "a duplicate content type was found" from SharePoint - which is
        /// the one answer that is definitely correct. A filtered query costs one request and settles
        /// it.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>Through CSOM, not <c>IContentType.UpdateAsync()</c>.</b> PnP Core can update the
        /// object <c>ContentTypes.AddAsync</c> returns but not one read from <c>Web.ContentTypes</c>
        /// - the latter fails with <i>"An unexpected 'PrimitiveValue' node was found when reading
        /// from the JSON reader"</i>. A provisioning run is almost entirely the second case. It also
        /// gets <c>UpdateChildren</c>, which <c>UpdateAsync()</c> cannot express at all. See
        /// <see cref="UpdateContentTypeRequest"/>.</para>
        /// <para>Only properties that actually differ are sent, so re-applying an unchanged template
        /// is a no-op rather than a write.</para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// These are the modern replacement for the custom form urls above, and unlike those they
        /// are allowed on a NoScript site - a form customizer is a registered SPFx component, not a
        /// page someone uploaded.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <paramref name="templateValue"/> is checked separately from <paramref name="wanted"/>:
        /// a template that does not mention a property must not blank it, and a token that resolves
        /// to nothing is not the same as a property the template left out.
        /// </remarks>
        ...
        /// <remarks>
        /// Each field ref is handled in the pass that matches the column it points at, so a link to a
        /// lookup column is created in the second pass - after <c>ObjectField</c> has made it. A ref
        /// whose column is not in the template at all is treated as a standard column, because it
        /// must already exist on the site.
        /// </remarks>
            ...
            IContentType contentType, ContentTypeModel templateContentType, TokenParser parser)
        {
            ...
            foreach (FieldRefModel fieldRef in templateContentType.FieldRefs)
            {
                ...
                if (field == null)
                {
                    string message = $"The column '{fieldRef.Name ?? fieldRef.Id.ToString()}' is not on this site, so it was not added to content type '{contentType.Name}'.";
                    context.Logger?.LogWarning("{Source}: {Message}", Constants.LOGGING_SOURCE, message);
                    WriteMessage(message, ProvisioningMessageType.Warning);
                    continue;
                }
        ...
        /// </summary>
        /// <remarks>
        /// <b>Not cosmetic, and not available in PnP Core.</b> A content type that inherits from
        /// another gets the parent's columns first with its own appended, so without this every
        /// derived content type comes out in an order the template did not ask for - and that order
        /// is what the new, edit and display forms use. See <see cref="ReorderFieldLinksRequest"/>.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para>Backlog <b>T15</b>, and it needed less than expected: <c>IDocumentSet</c> models
        /// allowed content types, shared columns, welcome page columns and default contents, and
        /// <c>UpdateAsync</c> takes them all in one call.</para>
        /// <para><c>KeepExisting*</c> is set from the template's <c>RemoveExistingContentTypes</c>,
        /// which is the only knob the schema offers - PnP Framework instead added and removed items
        /// individually, which cannot express "replace the set".</para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>Document sets are a feature, not a given.</b> Without the Document Sets site
        /// collection feature the parent content type <c>0x0120D520</c> does not exist, and creating
        /// one fails with <i>"The parent content type specified by content type identifier
        /// 0x0120D520 does not exist"</i> - which reads like a malformed id rather than a missing
        /// feature, and cost a round of diagnosis to tell apart.</para>
        /// <para>PnP Framework left the feature to the template. Activating it here is the better
        /// behaviour: a template that declares a document set has already said what it wants, and a
        /// template that would work on one site and not another for an invisible reason is worse
        /// than one that turns the feature on.</para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// A reference that resolves to nothing is reported. Silently dropping it produces a document
        /// set that is created, looks provisioned, and is missing the columns the template asked for
        /// - which is the failure mode this migration keeps meeting.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// PnP Framework uploaded the file into the content type's <c>_cts/&lt;name&gt;</c> folder
        /// itself. That is file provisioning, which <c>ObjectFiles</c> owns and which lands later in
        /// this phase - so the property is set and the upload is reported rather than done twice.
        /// </remarks>
