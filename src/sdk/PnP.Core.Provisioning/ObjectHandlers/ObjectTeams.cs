...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T11</b>, phase 8 - the last handler, and on paper the largest single item at
    /// 2,072 lines. Spike <b>S2</b> found why that number misleads: most of that file is raw HTTP
    /// plumbing that re-acquires an access token before every request and hand-polls for
    /// provisioning to finish. PnP Core's authentication and <c>ITeamManager</c> do both.</para>
    /// <para><b>Three ways a template can produce a team</b>, and they are mutually exclusive:
    /// <c>CloneFrom</c> copies an existing one, <c>GroupId</c> promotes an existing Microsoft 365
    /// group, and neither creates one from scratch. Only the middle case maps onto
    /// <c>ITeamManager.CreateTeamAsync</c>, which teamifies the group its context is bound to - the
    /// other two are Graph calls that PnP Core does not model.</para>
    /// <para><b>Configuration is written to Graph directly rather than through <c>ITeam</c>.</b> The
    /// provisioning schema's team model is a near mirror of Graph's <c>team</c> resource, so writing
    /// it is a one-to-one mapping; routing it through <c>ITeam</c> would mean translating every field
    /// twice and converting between two independently declared copies of each enum. <c>ITeam</c> is
    /// used where it is genuinely better - archiving, which returns an operation to wait on instead
    /// of a poll loop.</para>
    /// </remarks>
    internal partial class ObjectTeams : ObjectHierarchyHandlerBase
    {
        ...
        /// <remarks>
        /// A team exists before it is ready: channels and tabs added too early fail with a 404 on the
        /// team itself. PnP Framework polls the same way, for the same reason.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <b>Matched on mail nickname, not display name.</b> The nickname is the group's unique key
        /// and what actually collides; two teams may legitimately share a display name.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <b>This is the one creation mode PnP Core models.</b> <c>ITeamManager.CreateTeamAsync</c>
        /// teamifies the group its context is bound to, and it handles the provisioning wait that the
        /// other two paths have to poll for by hand.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <c>teams/{id}/clone</c> answers 202 with a <c>Location</c> header rather than a body, so
        /// the new team is found afterwards by the mail nickname the request asked for.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// The JSON is the caller's, passed through rather than modelled - that is the point of the
        /// element. Only the three properties the schema lets a template override are replaced.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// The schema's team is a near mirror of Graph's, which is why this is a mapping rather than
        /// a translation. Settings groups are only emitted when the template declares them - sending
        /// an empty settings object resets every value in it to its default.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <b>A team exists before it is usable.</b> Adding a channel or a tab too early fails with a
        /// 404 on the team itself, which reads as "the team was not created" rather than "not yet".
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Creating and cloning a team are both asynchronous and answer without an id, so the only
        /// way back to the new team is the nickname the request asked for.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// The same limitation as a user's photo in <c>ObjectAzureActiveDirectory</c>: the Graph call
        /// is a binary <c>PUT</c> and <c>ApiRequest.Body</c> is a string. Base64 encoding it would be
        /// accepted and would store the base64 text as the image - a run that reports success and
        /// leaves a corrupt photo. Reported instead. PnP Framework's own note says a team photo
        /// cannot be set before its mailbox exists anyway.
        /// </remarks>
