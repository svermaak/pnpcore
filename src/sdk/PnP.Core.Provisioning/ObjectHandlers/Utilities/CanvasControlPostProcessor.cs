...
namespace PnP.Core.Provisioning.ObjectHandlers.Utilities
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Only the List web part needs this today. It stores <c>selectedListId</c>, which is a
    /// GUID that means nothing on another site - so the list is re-resolved by url (or title, or the
    /// old id if it happens to still exist) and both the id and the url are rewritten.</para>
    /// <para>PnP Framework modelled this as a factory returning one of two strategy classes, one of
    /// which did nothing. A single method with an early return says the same thing.</para>
    /// </remarks>
    internal static class CanvasControlPostProcessor
    {
        ...
        /// </summary>
        /// <remarks>
        /// The url is tried first deliberately: it is the only one of the three that survives being
        /// applied to a different site, which is the whole point of this pass.
        /// </remarks>
        private static async Task<IList> ResolveListAsync(PnPContext context, JsonObject properties)
        {
            ...
            if (Guid.TryParse(listId, out Guid id) && id != Guid.Empty)
            {
                ...
                {
                    // Not on this site - fall through to the title.
                }
            }
        ...
        private static async Task<IList> GetListByUrlAsync(PnPContext context, string listUrl)
        {
            try
            {
                // The stored url may be server relative or site relative; GetListByServerRelativeUrl
                // needs the former.
