...
namespace PnP.Core.Provisioning.ObjectHandlers.Utilities
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>PnP Framework kept this in <c>Extensions/SecurityExtensions.cs</c> as an extension on
    /// the CSOM <c>SecurableObject</c>. PnP Core has the same abstraction -
    /// <see cref="ISecurableObject"/> - so one implementation still covers every scope. It lives
    /// here rather than inside a handler because <c>ObjectListInstance</c>, <c>ObjectFiles</c> and
    /// <c>ObjectSiteSecurity</c> all need it.</para>
    /// <para><b>Resolving a principal is the awkward part.</b> A template names a principal as a
    /// string - a login name, a UPN, or a SharePoint group title - and PnP Core's
    /// <see cref="IRoleAssignment"/> exposes only a numeric <c>PrincipalId</c>, with no navigation
    /// back to the member. So both directions need the site's users and groups in hand:
    /// applying resolves name → id, extracting resolves id → name.</para>
    /// </remarks>
    internal static class SecurityUtilities
    {
        ...
        /// </summary>
        /// <remarks>
        /// An object that inherits has no security of its own to describe, and emitting an empty
        /// <c>&lt;pnp:Security&gt;</c> for it would make a re-apply <em>break</em> inheritance -
        /// turning an extract-then-apply round trip into a change.
        /// </remarks>
