...
namespace PnP.Core.Provisioning.ObjectHandlers.Utilities
{
    ...
    internal static class FieldUtilities
    {
        ...
        /// </summary>
        /// <remarks>
        /// <para>A template may point a lookup at its target by web-relative url rather than by id -
        /// which is the only portable way to write one, since the id is generated per site.
        /// SharePoint's <c>AddFieldAsXml</c> only accepts an id, so the url is resolved here.</para>
        /// <para>The id is written in <b>braced</b> form. SharePoint accepts both, but the field
        /// schema it hands back uses braces, so matching that keeps a re-extracted template
        /// byte-identical to the one that was applied.</para>
        /// </remarks>
