...
namespace PnP.Core.Provisioning.ObjectHandlers.Utilities
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>A <c>&lt;pnp:DataValue&gt;</c> is always a string, because XML has nothing else. What it
    /// has to become depends entirely on the target column's type: a person column wants a principal
    /// id, a lookup wants an item id, a managed metadata column wants a term guid <em>and</em> its
    /// label, and a url wants a url plus a description. Sending the string through unchanged either
    /// fails or - worse for a lookup - silently stores nothing.</para>
    /// <para>PnP Framework did this against CSOM's <c>FieldUserValue</c> family. PnP Core has the
    /// same shapes, built through factory methods on <see cref="IField"/> so the value carries the
    /// field it belongs to.</para>
    /// <para><b>Multi-value separators are ambiguous and always have been.</b> A comma or a semicolon
    /// separates entries for people, lookups and taxonomy, which means a display name containing one
    /// cannot be expressed. That is inherited behaviour, not a decision taken here.</para>
    /// </remarks>
    internal static class ListItemUtilities
    {
            ...
            IDictionary<string, string> values, TokenParser parser, Action<string> reportWarning = null)
        {
            ...
            foreach (KeyValuePair<string, string> entry in values)
            {
                ...
                // ID is routinely used as the key column of a data row set, so it arrives here as a
                // value even though it can never be written. Setting it fails the whole item.
        ...
        private static async Task<object> CoerceAsync(PnPContext context, IField field, string value)
        {
            switch (field.TypeAsString)
            {
                ...
                case "MultiChoice":
                    // The schema's separator for a multi-choice column is ";#", which is also how
                    // SharePoint itself renders one, so an extracted value round-trips.
                ...
                case "Boolean":
                    ...
                    // A template writes TRUE/FALSE as often as true/false, and "1"/"0" occurs too.
        ...
        private static async Task<object> CoerceUserAsync(PnPContext context, IField field, string value)
        {
            ...
            {
                // A numeric entry is already a site user id; anything else is a login name that has
                // to be provisioned into the site's user list before it can be referenced.
        ...
        private static object CoerceLookup(IField field, string value)
        {
            ...
            foreach (string part in value.Split(',', ';').Select(p => p.Trim()).Where(p => p.Length > 0))
            {
                // A lookup is addressed by the target item's id and by nothing else - there is no
                // by-title form. A template carrying a title here is a template that silently
                // stores nothing, so this throws rather than guessing.
        ...
        /// </summary>
        /// <remarks>
        /// <para>A path is walked and yields both the id and the label. <b>A guid yields the id
        /// only</b>: Graph has no store-level term collection, so finding a term from its id alone
        /// would mean enumerating every term of every set on the site - which is what the id form
        /// exists to avoid. The label is left empty and SharePoint resolves it from the guid.</para>
        /// <para>Prefer the path form in a template regardless: a guid is tenant-specific, and a
        /// template carrying one is not portable.</para>
        /// </remarks>
        ...
        private static object CoerceUrl(IField field, string value)
        {
            ...
            // "url, description" is the schema's form. A value containing a separator but not
            // exactly two parts is passed through as a plain url rather than guessed at.
