...
namespace PnP.Core.Provisioning.ObjectHandlers.Utilities
{
    ...
    /// </summary>
    /// <remarks>
    /// A directory element is a convenience in the schema, not a thing that exists on the server:
    /// the engine turns it into one <c>&lt;pnp:File&gt;</c> per file found in the connector, and
    /// from there the upload path is identical. Doing that expansion up front is what keeps
    /// <c>ObjectFiles</c> from having two code paths that drift apart.
    /// </remarks>
    internal static class TemplateFileUtilities
    {
        ...
        /// </summary>
        /// <remarks>
        /// <b>The source and target paths are carried as parameters</b> rather than mutated on the
        /// model. PnP Framework recursed by assigning to <c>directory.Src</c> and
        /// <c>directory.Folder</c> and then trimming them back afterwards, which mutates the caller's
        /// template as a side effect of reading it - and leaves it corrupted if anything throws
        /// mid-walk.
        /// </remarks>
