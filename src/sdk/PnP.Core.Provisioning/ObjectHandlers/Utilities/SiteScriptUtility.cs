...
namespace PnP.Core.Provisioning.ObjectHandlers.Utilities
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T7</b>. PnP Core only ever <em>consumes</em> a site design id, at site
    /// creation - there is no model for designs or scripts and no CRUD. PnP Framework reaches them
    /// through CSOM (<c>Tenant.GetSiteScripts</c>, <c>CreateSiteDesign</c> and friends), but the same
    /// operations have a documented REST surface under
    /// <c>Microsoft.SharePoint.Utilities.WebTemplateExtensions.SiteScriptUtility</c>, which is what
    /// this uses. That trades a dozen CSOM request classes for a dozen POST bodies.</para>
    /// <para><b>Everything here runs against the tenant admin site.</b> The endpoints exist on other
    /// webs and fail there in ways that read like permission problems.</para>
    /// <para>Kept deliberately thin - request in, parsed response out, no template concepts. What a
    /// template means by "overwrite" or "default" belongs to the handler, not here.</para>
    /// </remarks>
    internal static class SiteScriptUtility
    {
        ...
        /// </summary>
        /// <remarks>
        /// <c>grantedRights</c> is a string rather than a number in the payload: the endpoint models
        /// it as a 64 bit flag set, and JSON numbers of that width are rejected.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// A single quote is doubled, which is the OData rule - a title containing an apostrophe is
        /// ordinary and would otherwise terminate the literal and produce a parse error that names
        /// nothing useful.
        /// </remarks>
        ...
        internal sealed class SiteDesignMetadata
        {
            ...
            /// </summary>
            /// <remarks>
            /// Read by hand rather than deserialised: <c>SiteScriptIds</c> arrives as
            /// <c>{"__metadata":…,"results":["guid"]}</c>, not as an array, so a plain
            /// <c>List&lt;Guid&gt;</c> property binds to nothing and the design looks as though it
