...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// Backlog <b>T4</b> turned out to need no work: <c>IWeb.AddIndexedPropertyAsync</c> and
    /// <c>RemoveIndexedPropertyAsync</c> are already public and already handle the base64 encoding
    /// and <c>|</c> joining of <c>vti_indexedpropertykeys</c>.
    /// </remarks>
    internal class ObjectPropertyBagEntry : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// Writing to these can destabilise a site, so they are skipped unless the caller sets
        /// <c>OverwriteSystemPropertyBagValues</c>. Note this guards <b>apply</b> only - extraction
        /// deliberately captures everything, because a template's base-template diff is what
        /// decides which entries are worth keeping.
        /// </remarks>
        ...
        public override async Task<ProvisioningTemplate> ExtractObjectsAsync(PnPContext context, ProvisioningTemplate template, ExtractConfiguration configuration)
        {
            using (context.Logger?.BeginScope(Name))
            {
                ...
                // MIGRATION PHASE 9: PnP Framework diffed the result against the site's base
                // template here, dropping every entry SharePoint creates by default. That needs
                // BaseTemplateManager wired into ExtractConfiguration, which is a phase 9 item -
                // until then extraction is complete but noisy, which is the safer direction.
        ...
        public override async Task<TokenParser> ProvisionObjectsAsync(PnPContext context, ProvisioningTemplate template, TokenParser parser, ApplyConfiguration configuration)
        {
            using (context.Logger?.BeginScope(Name))
            {
                // A NoScript site rejects property bag writes outright. PnP Framework returned
                // early rather than letting every entry fail in turn, and so does this.
                ...
                foreach (PropertyBagEntryModel entry in template.PropertyBagEntries)
                {
                    ...
                    // Overwrite=false means "create if absent", not "always write".
                ...
                // not in the property bag yet, and returns false rather than throwing - which would
        ...
        /// </summary>
        /// <remarks>
        /// <c>IWeb</c> exposes add and remove but no getter, and extraction needs to know which
        /// entries carry the flag. The stored form is a <c>|</c>-delimited list of base64 encoded,
        /// <b>Unicode</b> key names - UTF-8 would decode to mojibake rather than failing, so the
        /// encoding matters.
        /// </remarks>
        private static List<string> GetIndexedPropertyKeys(IWeb web)
        {
            ...
            foreach (string encoded in raw.Split(new[] { "|" }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                {
                    // A malformed entry should not stop the rest being read.
                }
            }
