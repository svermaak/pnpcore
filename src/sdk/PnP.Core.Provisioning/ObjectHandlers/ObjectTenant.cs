...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Phase 8. PnP Framework's <c>ObjectTenant</c> is 86 lines that delegate everything to an
    /// 890-line <c>TenantHelper</c>, which is why the porting backlog under-counted it: the handler
    /// inventory saw 86 lines. The [T13] audit found that <b>most of it is already in PnP Core or is
    /// a single REST call</b>; only the CDN needs CSOM.</para>
    /// <para><b>Everything here runs against the tenant admin site</b>, obtained once and shared.
    /// Storage entities are the exception - they live on the app catalog site, and are written
    /// through a context bound to it.</para>
    /// <para><b>Not extracted.</b> <c>WillExtract</c> is false, matching PnP Framework: tenant
    /// settings are not properties of the site being extracted, and emitting them would make every
    /// extracted template carry the tenant's configuration.</para>
    /// </remarks>
    internal class ObjectTenant : ObjectHandlerBase
    {
        ...
        {
            // Tenant settings are not a property of the site being extracted.
            _willExtract ??= false;
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>Shared with <c>ObjectHierarchyTenant</c>.</b> The same element appears on a
        /// template and on a hierarchy, and PnP Framework has two handlers calling the identical
        /// eleven helpers - so the work lives here once and both call it.</para>
        /// <para>A connector is passed rather than a template because that is the only real
        /// difference between the two callers: a tenant template's files hang off the
        /// <em>hierarchy</em>'s connector. Nothing below needs anything else off a template.</para>
        /// </remarks>
            ...
            FileConnectorBase connector, TokenParser parser)
        {
            ...
            {
                // Without the admin site none of the below can run, and every one of them would
                // otherwise fail separately with the same underlying cause.
            ...
            using (admin)
            {
                // Scripts before designs: a design references its scripts by id, and the ids of
                // scripts this same template just created only exist as tokens afterwards.
        ...
        /// </summary>
        /// <remarks>
        /// <b>Said out loud rather than skipped.</b> A template that configures the CDN and gets no
        /// CDN and no message is worse than one that fails: the site looks provisioned. These are
        /// tracked in the backlog under T13 and are the next thing to land.
        /// </remarks>
            ...
            ProvisioningTenant tenant, TokenParser parser)
        {
            ...
            foreach (SiteScriptModel script in tenant.SiteScripts)
            {
                ...
                try
                {
                    ...
                    {
                        // Not an error, and the token still has to be published - a site design in
                        // the same template refers to this script by title and needs its id.
                        id = match.Id;
                    }
        ...
        /// </summary>
        /// <remarks>
        /// Tokens are resolved <em>after</em> the bytes are decoded, not before: the file is the
        /// thing on disk, and a token inside it is meant to be substituted into the script's content.
        /// </remarks>
        ...
        private async Task<TokenParser> ApplySiteDesignsAsync(PnPContext admin, ProvisioningTenant tenant, TokenParser parser)
        {
            ...
            foreach (SiteDesignModel design in tenant.SiteDesigns)
            {
                ...
                try
                {
                    ...
                    // The template names its scripts; the endpoint wants their ids. The tokens the
                    // site script pass published are what turns one into the other, which is the
                    // whole reason scripts are processed first.
        ...
        private async Task ApplyGrantsAsync(PnPContext admin, SiteDesignModel design, Guid id, TokenParser parser)
        {
            ...
            // Grouped by right: the endpoint takes a list of principals and one right, so granting
            // them one at a time would be a round trip per person.
        ...
        /// </summary>
        /// <remarks>
        /// <c>SiteDesignWebTemplate</c>'s members <em>are</em> SharePoint's ids - <c>TeamSite = 64</c>,
        /// <c>CommunicationSite = 68</c> - so the value is cast rather than mapped. A mapping table
        /// would be a second copy of the same numbers, free to drift from the enum.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para>All of this is already on <c>ITenantProperties</c> - the [T13] audit's main finding.
        /// The enum names line up between the two models, so they are converted by name rather than
        /// by a hand-written table that would silently drift when either side gains a member.</para>
        /// <para><b>Only the capability is written when sharing is disabled.</b> The rest describe how
        /// sharing behaves, and SharePoint rejects several of them outright when it is off. PnP
        /// Framework guards the same way.</para>
        /// </remarks>
        private async Task ApplySharingSettingsAsync(PnPContext admin, ProvisioningTenant tenant)
        {
            ...
            try
            {
                ...
                if (wanted.SharingCapability != Model.SharingCapability.Disabled)
                {
                    ...
                    // Space separated, which is the format the property takes - not a JSON array and
                    // not comma separated.
        ...
        /// </summary>
        /// <remarks>
        /// By name on purpose. The two enums are independently declared copies of the same SharePoint
        /// concept, and their numeric values are not guaranteed to agree - casting would compile and
        /// then quietly set the wrong thing.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <b>The only part of the tenant element that needs CSOM</b> - see
        /// <c>TenantCdnRequests</c>. Each CDN is independent, so one failing does not stop the other.
        /// </remarks>
        ...
        private async Task ApplyCdnAsync(PnPContext admin, CdnSettings settings, TenantCdnType type, TokenParser parser)
        {
            ...
            try
            {
                ...
                // Read before writing: flipping a CDN that is already in the wanted state is a
                // tenant-wide operation with a real cost, and PnP Framework guards the same way.
                ...
                if (!settings.Enabled)
                {
                    // Origins and policies on a disabled CDN are rejected, and would be meaningless
        ...
        private async Task ApplyCdnOriginsAsync(PnPContext admin, CdnSettings settings, TenantCdnType type, TokenParser parser)
        {
            ...
            foreach (CdnOrigin origin in settings.Origins)
            {
                ...
                try
                {
                    // Add what is missing and remove what is there. Asking for either when it is
                    // already so is not an error, but it is a tenant-wide round trip either way.
        ...
        /// </summary>
        /// <remarks>
        /// An absent policy in a template means "leave it alone", not "set it to empty" - so an empty
        /// value is dropped rather than written. Writing it would clear a tenant-wide setting the
        ...
        /// </summary>
        /// <remarks>
        /// The app catalog site, not the admin site - see <see cref="StorageEntities"/>. A tenant
        /// with no app catalog is reported rather than treated as a failure, because it is a tenant
        /// configuration question and not something the template got wrong.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>Single valued only</b>, matching PnP Framework - the schema has no way to express
        /// a multi-valued property, so there is nothing to lose by not supporting one.</para>
        /// <para>The account name is prefixed with the SharePoint claims moniker
        /// <c>i:0#.f|membership|</c>, which is how the profile store keys a user. Passing the bare
        /// UPN is accepted and matches nobody.</para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// <para><b>Approves, it does not request.</b> A permission request is raised by deploying an
        /// SPFx package that declares it; the template's job is to say which of the pending ones
        /// should be granted. So a template naming a permission nothing has requested has nothing to
        /// approve, and that is reported rather than treated as a failure.</para>
        /// <para>The service principal lives on the <b>tenant app catalog</b> site.</para>
        /// <para><b>Repeated until no request matches.</b> Deploying several packages that ask for
        /// the same scope raises several requests, and approving one leaves the rest pending - PnP
        /// Framework loops for the same reason.</para>
        /// </remarks>
        ...
        {
            // A grant's Scope is a space separated list of everything granted on that resource,
            // while a request's Scope is one scope - hence Contains rather than equality, which is
            // what PnP Framework does too.
        ...
        /// </summary>
        /// <remarks>
        /// <para>These are directory settings, not SharePoint settings. A tenant that has never
        /// changed them has <b>no <c>groupSettings</c> entry at all</b> - the values come from the
        /// <c>Group.Unified</c> template - so the first apply has to create the entry and later ones
        /// have to patch it. Getting that backwards produces a 404 on a fresh tenant and a duplicate
        /// on a configured one.</para>
        /// <para>Values not named by the template are left at whatever they already are, which for a
        /// newly created entry means the template's defaults.</para>
        /// </remarks>
            ...
            ReadGroupSettingsAsync(PnPContext admin)
        {
            ...
            foreach (JsonElement template in ItemsOf(templates.Response))
            {
                ...
                {
                    // The template's id is the templateId a new setting references, and its
                    // defaultValue entries are the starting point for one.
        ...
        private static IEnumerable<JsonElement> ItemsOf(string json)
        {
            ...
            using (JsonDocument document = JsonDocument.Parse(json))
            {
                ...
                foreach (JsonElement item in value.EnumerateArray())
                {
                    // Cloned, because the document is disposed when this method's iterator ends and
                    // a JsonElement is a view over it.
        ...
        /// </summary>
        /// <remarks>
        /// <para>A storage entity is a <b>web property bag entry on the app catalog site</b>, and
        /// NoScript blocks property bag writes. The app catalog is NoScript by default like every
        /// other modern site, so this is the common case rather than an exotic one - and SharePoint
        /// answers it with a bare <c>0x80070005 Access is denied</c>, which reads as a missing
        /// permission and sends the reader after the wrong thing.</para>
        /// <para>Checked rather than assumed: an access-denied from a genuinely missing permission
        /// must not be labelled a NoScript problem.</para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Replacing rather than adding: applying the same template twice would otherwise leave two
        /// tokens with the same name, and which one wins is an implementation detail of the parser.
        /// </remarks>
