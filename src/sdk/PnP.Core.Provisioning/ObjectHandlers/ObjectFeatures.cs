...
namespace PnP.Core.Provisioning.ObjectHandlers
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>Apply only.</b> As in PnP Framework, <c>WillExtract</c> returns <c>false</c>:
    /// extracting the active feature list would capture dozens of features SharePoint activates
    /// itself, which is noise in a template rather than intent.</para>
    /// <para>Feature activation is deliberately forgiving. A feature that is unavailable on the
    /// target site type, or already in the wanted state, must not fail the run - a template is
    /// frequently applied to sites that differ slightly from the one it came from.</para>
    /// </remarks>
    internal class ObjectFeatures : ObjectHandlerBase
    {
        ...
        /// </summary>
        /// <remarks>
        /// Only features whose state actually differs are touched. Re-activating an active feature
        /// is not merely wasteful - some features re-run their provisioning on activation, which
        /// would make re-applying a template destructive.
        /// </remarks>
