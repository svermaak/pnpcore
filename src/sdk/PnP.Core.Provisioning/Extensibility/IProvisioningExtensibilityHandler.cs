...
namespace PnP.Core.Provisioning.Extensibility
{
    ...
    /// </summary>
    /// <remarks>
    /// <b>Breaking change from PnP Framework.</b> The first parameter was a CSOM
    /// <c>ClientContext</c> and is now a <see cref="PnPContext"/>; both methods are async; and the
    /// <c>PnPMonitoredScope</c> parameter is an <see cref="ILogger"/>. Third-party handlers written
    /// against PnP Framework must be updated - there is no way to keep the old signature without
    /// dragging CSOM back in, which is the whole point of the migration.
    /// </remarks>
