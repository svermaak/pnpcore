...
namespace PnP.Core.Provisioning.Connectors
{
    ...
    /// </summary>
    /// <remarks>
    /// Rewritten from the PnP Framework version, which was built on CSOM. The behaviour is the same;
    /// the implementation differs in two ways worth knowing about:
    ///
    /// 1. **Folder resolution is direct.** The CSOM version resolved the document library with
    ///    <c>GetListByUrl</c>, read its <c>RootFolder.ServerRelativeUrl</c>, then walked sub folders one
    ///    call at a time. Since a container is documented as a URL path ("mydocs/myfolder"), the target
    ///    folder is simply <c>{web server relative url}/{container}</c>, which
    ///    <see cref="IWeb.GetFolderByServerRelativeUrlAsync"/> resolves in a single call.
    /// 2. **Listing uses the folder, not CAML.** <c>IFolder.Files</c> and <c>IFolder.Folders</c> replace
    ///    the hand-written CAML queries.
    ///
    /// <see cref="FileConnectorBase"/> is synchronous while PnP Core is async, so the overrides below
    /// wrap async implementations with <c>GetAwaiter().GetResult()</c> - the same approach PnP Core uses
    /// for its own synchronous twins.
    /// </remarks>
