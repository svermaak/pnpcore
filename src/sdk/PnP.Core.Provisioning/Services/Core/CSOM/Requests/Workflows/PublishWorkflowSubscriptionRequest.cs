...
{    /// <summary>
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T16</b>. Two CSOM methods behind one request:
    /// <c>PublishSubscription(subscription)</c> for a site workflow, and
    /// <c>PublishSubscriptionForList(subscription, listId)</c> for a list one. They differ only in
    /// whether a list is named, so keeping them apart would duplicate the whole subscription
    /// construction.</para>
    /// <para>Association-time properties go on through <c>SetProperty</c> method calls rather than
    /// property setters - <c>PropertyDefinitions</c> is a dictionary, and CSOM has no way to assign
    /// one wholesale.</para>
    /// </remarks>
