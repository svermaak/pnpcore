...
{    /// <summary>
    /// Publishes a saved workflow definition.
    /// </summary>
    /// <remarks>
    /// Backlog <b>T16</b>. <c>WorkflowDeploymentService.PublishDefinition(id)</c>. A definition
    /// must be published before a subscription can reference it, so this always follows
    /// <see cref="SaveWorkflowDefinitionRequest"/>.
    /// </remarks>
    internal sealed class PublishWorkflowDefinitionRequest : WorkflowRequestBase, IRequest<object>
    {
        private readonly Guid siteId;
        private readonly Guid webId;
        private readonly Guid definitionId;

        internal PublishWorkflowDefinitionRequest(Guid siteId, Guid webId, Guid definitionId)
        {
            this.siteId = siteId;
            this.webId = webId;
