...
{    /// <summary>
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T16</b>. Every workflow call starts by constructing a
    /// <c>WorkflowServicesManager(context, web)</c> and then asking it for one of three services -
    /// deployment (definitions), subscription (associations) or instance (running workflows). That
    /// prologue lives here.</para>
    /// <para><b>These are deprecated by Microsoft and disabled in many tenants.</b> Requests fail
    /// with a service-unavailable error rather than a clean "not supported", so callers should
    /// treat failure as "workflows are off here" and continue. Live tests must degrade with
    /// <c>Assert.Inconclusive</c>.</para>
    /// </remarks>
