...
{    /// <summary>
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T10</b>, per spike <b>S1</b>. <c>TermSetItem.CreateTerm(name, lcid, guid)</c>
    /// - <c>TermSetItem</c> being the shared base of <c>TermSet</c> and <c>Term</c>, which is why
    /// the same request creates both a root term and a child term; only the parent differs.</para>
    /// <para>This is the request S1 exists for. Graph's <c>ITermCollection.AddAsync(name, description)</c>
    /// has no id parameter and <c>ITerm.Id</c> is read-only, so a Graph-first path would reassign
    /// every term GUID a template declares.</para>
    /// </remarks>
