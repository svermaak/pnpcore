...
{    /// <summary>
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T10</b>, and spike <b>S1</b>'s divergence (d). CSOM has
    /// <c>TermStore.GetSiteCollectionGroup(site, createIfMissing)</c>, which both finds the group
    /// and binds it to a specific site. Graph has <c>TermGroupScope.SiteCollection</c> but nothing
    /// that ties a scoped group to a particular site collection - so
    /// <c>SiteCollectionTermGroupResolver</c> has to match by convention on the group id, and
    /// cannot create one at all.</para>
    /// <para>This request replaces both halves of that workaround for the handler path.
    /// The resolver stays for token resolution, which must remain read-only.</para>
    /// </remarks>
