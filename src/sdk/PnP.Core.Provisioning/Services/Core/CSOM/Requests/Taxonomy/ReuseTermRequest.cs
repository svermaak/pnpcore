...
{    /// <summary>
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T10</b>. Two related CSOM calls on <c>TermSetItem</c>:</para>
    /// <list type="bullet">
    /// <item><description><c>ReuseTerm(term, reuseBranch)</c> - reuse the term, and with
    /// <c>reuseBranch: true</c> its whole subtree.</description></item>
    /// <item><description><c>ReuseTermWithPinning(term)</c> - reuse it such that changes to the
    /// source propagate to this copy.</description></item>
    /// </list>
    /// <para>Spike <b>S1</b> found Graph's <c>ITermRelation</c> covers <c>Pin</c> and <c>Reuse</c>
    /// as relation kinds, but has no flag for <em>branch</em> reuse - so reusing a term with its
    /// descendants has no Graph expression. Since taxonomy is CSOM-first anyway (decision
    /// <b>D7</b>), both go through here.</para>
    /// </remarks>
