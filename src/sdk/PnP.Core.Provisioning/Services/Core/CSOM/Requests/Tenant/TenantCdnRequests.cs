...
namespace PnP.Core.Provisioning.Services.Core.CSOM.Requests.Tenant
{
    ...
    /// </summary>
    /// <remarks>
    /// The values are SharePoint's <c>SPOTenantCdnType</c>, sent as a CSOM <c>Enum</c> parameter.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// SharePoint's <c>SPOTenantCdnPolicyType</c>. The names matter as well as the numbers:
    /// <c>GetTenantCdnPolicies</c> answers with <c>Name;Value</c> strings keyed by these names.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T13</b>, and <b>the only part of it that needs CSOM</b>.
    /// <c>ITenantProperties</c> exposes three public-CDN properties and a read-only origins list;
    /// there is nothing for the private CDN, for adding or removing origins, or for policies, and no
    /// REST surface for any of it - the SharePoint Online Management Shell drives these through
    /// exactly the CSOM calls below.</para>
    /// <para>Every one of them hangs off <c>new Tenant(context)</c>, so the constructor path and the
    /// action bookkeeping live here once.</para>
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// The result is a <see cref="CdnEnabledInfo"/> rather than a <c>bool</c> because of decision
    /// <b>D11</b>: batching casts every request to <c>IRequest&lt;object&gt;</c>, and covariance
    /// does not apply to value types - <c>IRequest&lt;bool&gt;</c> compiles and then throws.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// One class for three methods because they differ only in name and arity, and three classes
    /// would be three copies of the same constructor path.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// <para>Origins are plain urls. <b>Policies are not</b>: each entry is
    /// <c>PolicyName;Value</c>, which is why <see cref="TenantCdnPolicies.Parse"/> exists rather
    /// than the caller indexing into the list.</para>
    /// </remarks>
    ...
    internal static class TenantCdnPolicies
    {
        ...
        /// </summary>
        /// <remarks>
        /// <b>Unrecognised entries are skipped, not thrown on.</b> PnP Framework parses the name with
        /// <c>Enum.Parse</c> and lets it throw, which means a policy type added to SharePoint after
        /// this code was written breaks reading <em>every</em> policy rather than just that one.
        /// </remarks>
