...
namespace PnP.Core.Provisioning.Services.Core.CSOM.Requests.Lists
{
    ...
    /// </summary>
    /// <remarks>
    /// <para><b>PnP Core cannot express any of that.</b> <c>IListCollection.AddAsync</c> takes a
    /// title and a template type and nothing else - the underlying REST endpoint has no url
    /// parameter, so this is a protocol limit rather than a missing overload.</para>
    /// <para>The url matters more than it looks. It is the list's real identity: templates set it
    /// independently of the title (<c>Title="My List"</c>, <c>Url="Lists/MyList"</c>), every
    /// <c>{listurl:…}</c> token resolves against it, lookup columns address their target by it, and
    /// renaming a list later does not change it. Letting SharePoint derive it from the title
    /// produces a site where those references silently point at nothing.</para>
    /// <para>The type id was read from the shipped CSOM assembly, not from documentation - see
    /// <see cref="CsomTypeIds"/>.</para>
    /// </remarks>
    internal sealed class CreateListRequest : IRequest<CreatedListInfo>
    {
        ...
        /// </summary>
        /// <remarks>
        /// A class rather than a bare <see cref="Guid"/> on purpose: PnP Core batches requests by
        /// casting them to <c>IRequest&lt;object&gt;</c>, and covariance does not apply to value
        /// types, so <c>IRequest&lt;Guid&gt;</c> compiles and then throws at run time.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Only the properties a provisioning template can set are emitted. <c>TemplateFeatureId</c>
        /// is left out when empty: sending <c>Guid.Empty</c> makes SharePoint look for a feature
        /// with that id rather than treating it as "no feature".
        /// </remarks>
