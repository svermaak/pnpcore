...
{    /// <summary>
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T18</b>. Two CSOM calls that only make sense together:
    /// <c>ImportWebPart(xml)</c> turns the <c>.webpart</c> / <c>.dwp</c> XML a template carries
    /// into a <c>WebPartDefinition</c>, then <c>AddWebPart(definition.WebPart, zoneId, zoneIndex)</c>
    /// places it. Splitting them across round trips would mean holding a server-side object between
    /// requests, which CSOM does not support - so they share one batch.</para>
    /// </remarks>
