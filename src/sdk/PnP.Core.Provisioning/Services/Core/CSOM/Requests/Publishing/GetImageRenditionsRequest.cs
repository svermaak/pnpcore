...
namespace PnP.Core.Provisioning.Services.Core.CSOM.Requests.Publishing
{
    ...
    /// </summary>
    /// <remarks>
    /// Backlog <b>T20</b>. <c>SiteImageRenditions.GetRenditions(context)</c> takes only the
    /// implicit context, so the static method carries no serialized parameters at all.
    /// Publishing-feature gated: on a site without it, the call fails and the caller should treat
    /// that as "no renditions" rather than an error.
    /// </remarks>
