...
namespace PnP.Core.Provisioning.Services.Core.CSOM.Requests.UserResources
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T6</b>. The read half of localization: used on extract to build a
    /// template's <c>&lt;pnp:Localizations&gt;</c>, and by the token parser to resolve a list's
    /// title in the site's main language.</para>
    /// <para><c>GetValueForUICulture</c> returns a <c>ClientResult&lt;string&gt;</c>, which
    /// serializes as a bare value in the response rather than as an object with properties - hence
    /// reading the result directly rather than looking for a named property.</para>
    /// </remarks>
