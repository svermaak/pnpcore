...
namespace PnP.Core.Provisioning.Services.Core.CSOM.Requests.InformationPolicy
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>Backlog <b>T22</b>. <c>ProjectPolicy.ApplyProjectPolicy(context, web, policy)</c> takes
    /// the policy as an <em>object</em>, not a name - so the request first re-runs
    /// <c>GetProjectPolicies</c> and indexes into the result, which is what PnP Framework did too.
    /// Both happen in one round trip, so the extra call is free.</para>
    /// <para>The caller supplies the index because the policy name has already been matched against
    /// the list read by <see cref="GetProjectPoliciesRequest"/>; doing the match here would mean
    /// parsing the policy list twice.</para>
    /// </remarks>
